...
import LineChartsweek_linechart  from "./LineChartsweek_linechart";
...
const Groupapi_call_over_week_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Design:AFVK:v1",componentId:"292a4c6b74654e069a6d91cf69f03bb6",from:"GroupApiCallOverWeekGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("week_linechart")){
      setweek_linechart709e7({...week_linechart709e7,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const api_call_over_week_group03bb6Ref = useRef<any>(null);
  const handleClearSearch = () => {
    api_call_over_week_group03bb6Ref.current?.setSearchParams();
    api_call_over_week_group03bb6Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(api_call_over_week_group03bb6) && Object.keys(api_call_over_week_group03bb6)?.length>0)
      {
        setapi_call_over_week_group03bb6({})
      }
    }else 
      prevRefreshRef.current= true
  }, [api_call_over_week_group03bb6Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("week_linechart") ?<LineChartsweek_linechart /* 709e7 */ encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default Groupapi_call_over_week_group
