...
const Cardmost_used_apis = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async():Promise<void>=>{
    ...
    try{
      ...
      settotal_api_calls_groupd4dee((pre:any)=>({...pre,most_used_apis:dfd_mongodb_maindashboard_dfd_v1Props[0]?.most_used_apis}));
  }
    }catch(err){
      console.log(err);
    }
...
useEffect(() => {
  ...
  if (most_used_apis2686b?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `7 / 25`,gridRow: `1 / 22`, gap:``, height: `100%`, overflow: 'auto'}} >
      ...
      disabled= {most_used_apis2686b?.isDisabled ? true : false}
      onClick={handleClick} 
      contentAlign={"center"}
      >
      {total_api_calls_groupd4dee?.most_used_apis?total_api_calls_groupd4dee?.most_used_apis:"0"}
      </Card>
    </div>
  )
}
