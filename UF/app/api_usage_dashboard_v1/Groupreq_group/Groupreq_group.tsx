...
import Icontot_req_icon  from "./Icontot_req_icon";
...
const Groupreq_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const {total_requests06c5a, settotal_requests06c5a}= useContext(TotalContext) as TotalContextProps;
  const {tot_req_icon1fa8f, settot_req_icon1fa8f}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Design:AFVK:v1",componentId:"338a929827b441b58a690143f2cdf5e7",from:"GroupReqGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("total_requests")){
      settotal_requests06c5a({...total_requests06c5a,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("tot_req_icon")){
      settot_req_icon1fa8f({...tot_req_icon1fa8f,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const req_groupdf5e7Ref = useRef<any>(null);
  const handleClearSearch = () => {
    req_groupdf5e7Ref.current?.setSearchParams();
    req_groupdf5e7Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(req_groupdf5e7) && Object.keys(req_groupdf5e7)?.length>0)
      {
        setreq_groupdf5e7({})
      }
    }else 
      prevRefreshRef.current= true
  }, [req_groupdf5e7Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("total_requests") ?<Cardtotal_requests  /* 06c5a */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
        {allowedControls.includes("tot_req_icon")?<Icontot_req_icon /* 1fa8f */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
    </div>
 )
}

export default Groupreq_group
