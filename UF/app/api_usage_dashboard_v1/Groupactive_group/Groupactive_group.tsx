...
import Iconactive_icon  from "./Iconactive_icon";
...
const Groupactive_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Design:AFVK:v1",componentId:"eeeb5c250aa74e2993d16229dce31e18",from:"GroupActiveGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("active_apis")){
      setactive_apisac162({...active_apisac162,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("active_icon")){
      setactive_icon42af9({...active_icon42af9,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const active_group31e18Ref = useRef<any>(null);
  const handleClearSearch = () => {
    active_group31e18Ref.current?.setSearchParams();
    active_group31e18Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(active_group31e18) && Object.keys(active_group31e18)?.length>0)
      {
        setactive_group31e18({})
      }
    }else 
      prevRefreshRef.current= true
  }, [active_group31e18Props?.refresh,token])
  ...
  return (
    <div 
      style={{          
        ...
        columnGap: '0px',
        backgroundColor:'',
        backgroundImage:"url('https://cdns3dfsdev.toruslowcode.com/torus/9.1/CT009/resources/images/newwwww2.jpg')",
        backgroundPosition: '',
        backgroundSize: '',
        backgroundRepeat: '',
    ...
    >
        {allowedControls.includes("active_apis") ?<Cardactive_apis  /* ac162 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
        {allowedControls.includes("active_icon")?<Iconactive_icon /* 42af9 */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
    </div>
 )
}

export default Groupactive_group
