...
import LineChartsapi_call_over_hour_linechart  from "./LineChartsapi_call_over_hour_linechart";
...
const Groupapi_call_over_hour_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const {api_call_over_hour_linechart38cfd, setapi_call_over_hour_linechart38cfd}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Design:AFVK:v1",componentId:"4d87575ab35747028e3c1704e5612590",from:"GroupApiCallOverHourGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("api_call_over_hour_linechart")){
      setapi_call_over_hour_linechart38cfd({...api_call_over_hour_linechart38cfd,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const api_call_over_hour_group12590Ref = useRef<any>(null);
  const handleClearSearch = () => {
    api_call_over_hour_group12590Ref.current?.setSearchParams();
    api_call_over_hour_group12590Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(api_call_over_hour_group12590) && Object.keys(api_call_over_hour_group12590)?.length>0)
      {
        setapi_call_over_hour_group12590({})
      }
    }else 
      prevRefreshRef.current= true
  }, [api_call_over_hour_group12590Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("api_call_over_hour_linechart") ?<LineChartsapi_call_over_hour_linechart /* 38cfd */ encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default Groupapi_call_over_hour_group
