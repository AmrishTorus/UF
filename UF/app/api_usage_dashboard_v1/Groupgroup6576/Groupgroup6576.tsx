...
import Texttextwrwer  from "./Texttextwrwer";
import Iconicon234234  from "./Iconicon234234";
import Texttextwerwer  from "./Texttextwerwer";
...
const Groupgroup6576 = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const {textwrwer0e4e1, settextwrwer0e4e1}= useContext(TotalContext) as TotalContextProps;
  const {icon234234e2c9a, seticon234234e2c9a}= useContext(TotalContext) as TotalContextProps;
  const {textwerweraf6e8, settextwerweraf6e8}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Design:AFVK:v1",componentId:"1d3a3c21edab4f3a88dd5a1631d622ab",from:"GroupGroup6576",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("textwrwer")){
      settextwrwer0e4e1({...textwrwer0e4e1,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("icon234234")){
      seticon234234e2c9a({...icon234234e2c9a,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("textwerwer")){
      settextwerweraf6e8({...textwerweraf6e8,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const group6576622abRef = useRef<any>(null);
  const handleClearSearch = () => {
    group6576622abRef.current?.setSearchParams();
    group6576622abRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(group6576622ab) && Object.keys(group6576622ab)?.length>0)
      {
        setgroup6576622ab({})
      }
    }else 
      prevRefreshRef.current= true
  }, [group6576622abProps?.refresh,token])
  ...
  return (
    ...
    >
          {allowedControls.includes("textwrwer") ?<Texttextwrwer   /* 0e4e1 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
        {allowedControls.includes("icon234234")?<Iconicon234234 /* e2c9a */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
          {allowedControls.includes("textwerwer") ?<Texttextwerwer   /* af6e8 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default Groupgroup6576
