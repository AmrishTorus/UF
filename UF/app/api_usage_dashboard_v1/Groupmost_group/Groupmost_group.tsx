...
import Iconerror_rate_icon  from "./Iconerror_rate_icon";
...
const Groupmost_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Design:AFVK:v1",componentId:"e46460169e424963a78d04eee5ec5ce0",from:"GroupMostGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("error_rate")){
      seterror_rate72497({...error_rate72497,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("error_rate_icon")){
      seterror_rate_icon89a9a({...error_rate_icon89a9a,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  return (
    ...
    >
        {allowedControls.includes("error_rate") ?<Carderror_rate  /* 72497 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
        {allowedControls.includes("error_rate_icon")?<Iconerror_rate_icon /* 89a9a */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
    </div>
 )
}
