...
import Texttext34572547  from "./Texttext34572547";
import Iconicon8698  from "./Iconicon8698";
...
const Groupgroup79679 = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const {text3457254757f70, settext3457254757f70}= useContext(TotalContext) as TotalContextProps;
  const {icon86986b1a3, seticon86986b1a3}= useContext(TotalContext) as TotalContextProps;
  const {text2668f, settext2668f}= useContext(TotalContext) as TotalContextProps;
  //////////////
  const [open, setOpen] = React.useState(false);
  async function securityCheck() {
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Design:AFVK:v1",componentId:"56f0779d27dc48b69c22ce9fc508bff3",from:"GroupGroup79679",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("text34572547")){
      settext3457254757f70({...text3457254757f70,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("icon8698")){
      seticon86986b1a3({...icon86986b1a3,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("text")){
      settext2668f({...text2668f,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const group796798bff3Ref = useRef<any>(null);
  const handleClearSearch = () => {
    group796798bff3Ref.current?.setSearchParams();
    group796798bff3Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(group796798bff3) && Object.keys(group796798bff3)?.length>0)
      {
        setgroup796798bff3({})
      }
    }else 
      prevRefreshRef.current= true
  }, [group796798bff3Props?.refresh,token])
  ...
  return (
    ...
    >
          {allowedControls.includes("text34572547") ?<Texttext34572547   /* 57f70 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
        {allowedControls.includes("icon8698")?<Iconicon8698 /* 6b1a3 */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
          {allowedControls.includes("text") ?<Texttext   /* 2668f */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default Groupgroup79679
