...
const Groupconnected_application = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Design:AFVK:v1",componentId:"cc4cb5d37dab4218b60ceaba56b19ab2",from:"GroupConnectedApplication",isTable : true,accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("app_name")){
      setapp_namedc4c5({...app_namedc4c5,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("tppname")){
      settppname5b032({...tppname5b032,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("type")){
      settyped4eac({...typed4eac,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("status_value")){
      setstatus_value3beb3({...status_value3beb3,isDisabled:true});
    }
  //////////////
  }
  ...
  const connected_application19ab2Ref = useRef<any>(null);
  const handleClearSearch = () => {
    connected_application19ab2Ref.current?.setSearchParams();
    connected_application19ab2Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(connected_application19ab2) && Object.keys(connected_application19ab2)?.length>0)
      {
        setconnected_application19ab2({})
      }
    }else 
      prevRefreshRef.current= true
  }, [connected_application19ab2Props?.refresh,token])
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tableconnected_application headerButtonsRenders={renderBUttons}
        ...
        lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={connected_application19ab2Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData}/>}
      </div>
      </div>
    </div>
 )
