  ...
  const GetTableDetails = async () => {
    const orchestrationData = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        componentId: 'cc4cb5d37dab4218b60ceaba56b19ab2',
        isTable: true,
        from :"Table",
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Design:AFVK:v1:UO', 
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( connected_application19ab2Props?.presetValues&&Object.keys(connected_application19ab2Props?.presetValues).length > 0) {
          ...
          setconnected_application19ab2(uf_paginationDataFilter.data||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  const handlePrimaryTable = () => {
    ...
    {
      let data = connected_application19ab2[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "",
        value: data[""],
