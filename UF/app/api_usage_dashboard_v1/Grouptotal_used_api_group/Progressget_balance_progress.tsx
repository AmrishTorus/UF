...
const Progressget_balance_progress = ({encryptionFlagCompData, isDynamic, index, item}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "977363dd20b540af8ed8f5f4604a0d54",
          isTable: false,
    ...
    let temp:any=""
     if ("hasLogicCenter" in dfd_tob_total_used_api_dfd_v1Props && !dfd_tob_total_used_api_dfd_v1Props.hasLogicCenter) {
        ...
        {
          temp = api_paginationData.data.records[0]?.percentage1
        }
      }else{
      if(Array.isArray(dfd_tob_total_used_api_dfd_v1Props) && dfd_tob_total_used_api_dfd_v1Props?.length){
        temp = dfd_tob_total_used_api_dfd_v1Props[0]?.percentage1
      }
      }
    ...
    settotal_used_api_groupcd37d((pre:any)=>({...pre,percentage1:temp}))
    handleCustomCode()
  }

useEffect(()=>{
  let temp:any = dfd_tob_total_used_api_dfd_v1Props[0]?.percentage1
  if (typeof temp != 'number') {
    temp = 0
  } else {
    if (temp < 100) {
  ...
  settotal_used_api_groupcd37d((pre:any)=>({...pre,percentage1:temp}))
},[dfd_tob_total_used_api_dfd_v1Props[0]?.percentage1])
  ...
  useEffect(()=>{
    handleMapperValue()
  },[get_balance_progressa0d54?.refresh])

  if (get_balance_progressa0d54?.isHidden) {
    return <></>
  }

return (
  <div 
    style={{gridColumn: `10 / 25`,gridRow: `35 / 46`, gap:``, height: `100%`, overflow: 'auto'}} >
    <Progress 
      className=""
        theme = {'danger'}
        value = {isDynamic ? item?.percentage1 : (total_used_api_groupcd37d?.percentage1 || 0)}
    />
  </div>
  )
}

export default Progressget_balance_progress
