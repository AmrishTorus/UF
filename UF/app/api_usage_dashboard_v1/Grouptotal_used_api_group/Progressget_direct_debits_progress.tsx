...
const Progressget_direct_debits_progress = ({encryptionFlagCompData, isDynamic, index, item}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "2b35341e08714e4aad70c28b6b104032",
          isTable: false,
    ...
    let temp:any=""
     if ("hasLogicCenter" in dfd_tob_total_used_api_dfd_v1Props && !dfd_tob_total_used_api_dfd_v1Props.hasLogicCenter) {
        ...
        {
          temp = api_paginationData.data.records[0]?.percentage4
        }
      }else{
      if(Array.isArray(dfd_tob_total_used_api_dfd_v1Props) && dfd_tob_total_used_api_dfd_v1Props?.length){
        temp = dfd_tob_total_used_api_dfd_v1Props[0]?.percentage4
      }
      }
    ...
    settotal_used_api_groupcd37d((pre:any)=>({...pre,percentage4:temp}))
    handleCustomCode()
  }

useEffect(()=>{
  let temp:any = dfd_tob_total_used_api_dfd_v1Props[0]?.percentage4
  if (typeof temp != 'number') {
    temp = 0
  } else {
    if (temp < 100) {
  ...
  settotal_used_api_groupcd37d((pre:any)=>({...pre,percentage4:temp}))
},[dfd_tob_total_used_api_dfd_v1Props[0]?.percentage4])
  ...
  useEffect(()=>{
    handleMapperValue()
  },[get_direct_debits_progress04032?.refresh])

  if (get_direct_debits_progress04032?.isHidden) {
    return <></>
  }

return (
  <div 
    style={{gridColumn: `10 / 25`,gridRow: `48 / 60`, gap:``, height: `100%`, overflow: 'auto'}} >
    <Progress 
      className=""
        theme = {'misc'}
        value = {isDynamic ? item?.percentage4 : (total_used_api_groupcd37d?.percentage4 || 0)}
    />
  </div>
  )
}

export default Progressget_direct_debits_progress
