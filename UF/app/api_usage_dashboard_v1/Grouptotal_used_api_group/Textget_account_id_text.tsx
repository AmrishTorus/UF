...
const Textget_account_id_text = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  useEffect(()=>{
    handleMapperValue()
  },[get_account_id_textcfcd9?.refresh])

  if (get_account_id_textcfcd9?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 10`,gridRow: `23 / 33`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className="bg-white "
  variant="subheader-1"
  color="primary"
>
      {keyset("Get Account ID")}
</Text>
  </div>
  )
}

export default Textget_account_id_text
