...
const Progressget_acc_progress = ({encryptionFlagCompData, isDynamic, index, item}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "07ec78dbcfc44f4581c6606a8e7f3140",
          isTable: false,
    ...
    let temp:any=""
     if ("hasLogicCenter" in dfd_tob_total_used_api_dfd_v1Props && !dfd_tob_total_used_api_dfd_v1Props.hasLogicCenter) {
        ...
        {
          temp = api_paginationData.data.records[0]?.percentage2
        }
      }else{
      if(Array.isArray(dfd_tob_total_used_api_dfd_v1Props) && dfd_tob_total_used_api_dfd_v1Props?.length){
        temp = dfd_tob_total_used_api_dfd_v1Props[0]?.percentage2
      }
      }
    ...
    settotal_used_api_groupcd37d((pre:any)=>({...pre,percentage2:temp}))
    handleCustomCode()
  }

useEffect(()=>{
  let temp:any = dfd_tob_total_used_api_dfd_v1Props[0]?.percentage2
  if (typeof temp != 'number') {
    temp = 0
  } else {
    if (temp < 100) {
  ...
  settotal_used_api_groupcd37d((pre:any)=>({...pre,percentage2:temp}))
},[dfd_tob_total_used_api_dfd_v1Props[0]?.percentage2])
  ...
  useEffect(()=>{
    handleMapperValue()
  },[get_acc_progressf3140?.refresh])

  if (get_acc_progressf3140?.isHidden) {
    return <></>
  }

return (
  <div 
    style={{gridColumn: `10 / 25`,gridRow: `12 / 21`, gap:``, height: `100%`, overflow: 'auto'}} >
    <Progress 
      className=""
        theme = {'info'}
        value = {isDynamic ? item?.percentage2 : (total_used_api_groupcd37d?.percentage2 || 0)}
    />
  </div>
  )
}

export default Progressget_acc_progress
