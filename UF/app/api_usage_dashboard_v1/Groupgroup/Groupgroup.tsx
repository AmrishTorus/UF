...
import Texttext4545  from "./Texttext4545";
import Iconicon5675  from "./Iconicon5675";
...
const Groupgroup = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Design:AFVK:v1",componentId:"ed35f907cd59456a97cdbdca7d1657d5",from:"GroupGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("text4545")){
      settext454513feb({...text454513feb,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("icon5675")){
      seticon5675ee8ba({...icon5675ee8ba,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("text45645")){
      settext4564580602({...text4564580602,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const group657d5Ref = useRef<any>(null);
  const handleClearSearch = () => {
    group657d5Ref.current?.setSearchParams();
    group657d5Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(group657d5) && Object.keys(group657d5)?.length>0)
      {
        setgroup657d5({})
      }
    }else 
      prevRefreshRef.current= true
  }, [group657d5Props?.refresh,token])
  ...
  return (
    ...
    >
          {allowedControls.includes("text4545") ?<Texttext4545   /* 13feb */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
        {allowedControls.includes("icon5675")?<Iconicon5675 /* ee8ba */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
          {allowedControls.includes("text45645") ?<Texttext45645   /* 80602 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}
