...
import Groupgroup454  from "../Groupgroup454/Groupgroup454";
import Groupgroup  from "../Groupgroup/Groupgroup";
import Groupgroup6576  from "../Groupgroup6576/Groupgroup6576";
import Groupgroup79679  from "../Groupgroup79679/Groupgroup79679";
...
import Textconsent_lifecycles  from "./Textconsent_lifecycles";
...
const Groupgroup123 = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const {consent_lifecycles11691, setconsent_lifecycles11691}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Design:AFVK:v1",componentId:"e7a22cb9c1da4891b40a25f8e5a3a04c",from:"GroupGroup123",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("consent_lifecycles")){
      setconsent_lifecycles11691({...consent_lifecycles11691,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("group454")){
      setgroup4549ff98({...group4549ff98,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("group")){
      setgroup657d5({...group657d5,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("group6576")){
      setgroup6576622ab({...group6576622ab,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("group79679")){
      setgroup796798bff3({...group796798bff3,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const group1233a04cRef = useRef<any>(null);
  const handleClearSearch = () => {
    group1233a04cRef.current?.setSearchParams();
    group1233a04cRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(group1233a04c) && Object.keys(group1233a04c)?.length>0)
      {
        setgroup1233a04c({})
      }
    }else 
      prevRefreshRef.current= true
  }, [group1233a04cProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("group454")  &&<Groupgroup454  
        ...
        {allowedComponent.includes("group")  &&<Groupgroup  
          ...
          paginationDetails={paginationDetails}
          setIsProcessing={setIsProcessing}        />}
        {allowedComponent.includes("group6576")  &&<Groupgroup6576  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          primaryTableData={primaryTableData}
          setPrimaryTableData={setPrimaryTableData}
          ...
          paginationDetails={paginationDetails}
          setIsProcessing={setIsProcessing}        />}
        {allowedComponent.includes("group79679")  &&<Groupgroup79679  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          primaryTableData={primaryTableData}
          setPrimaryTableData={setPrimaryTableData}
          ...
          paginationDetails={paginationDetails}
          setIsProcessing={setIsProcessing}        />}
          {allowedControls.includes("consent_lifecycles") ?<Textconsent_lifecycles   /* 11691 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default Groupgroup123
