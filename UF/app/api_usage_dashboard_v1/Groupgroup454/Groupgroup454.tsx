...
import Texttext45645  from "./Texttext45645";
import Iconicon5645  from "./Iconicon5645";
import Texttext23523  from "./Texttext23523";
...
const Groupgroup454 = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const {text45645c95a7, settext45645c95a7}= useContext(TotalContext) as TotalContextProps;
  const {icon56454d73c, seticon56454d73c}= useContext(TotalContext) as TotalContextProps;
  const {text23523e7140, settext23523e7140}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Design:AFVK:v1",componentId:"98c73da3ec924b46a87aa54c7119ff98",from:"GroupGroup454",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("text45645")){
      settext45645c95a7({...text45645c95a7,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("icon5645")){
      seticon56454d73c({...icon56454d73c,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("text23523")){
      settext23523e7140({...text23523e7140,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const group4549ff98Ref = useRef<any>(null);
  const handleClearSearch = () => {
    group4549ff98Ref.current?.setSearchParams();
    group4549ff98Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(group4549ff98) && Object.keys(group4549ff98)?.length>0)
      {
        setgroup4549ff98({})
      }
    }else 
      prevRefreshRef.current= true
  }, [group4549ff98Props?.refresh,token])
  ...
  return (
    ...
    >
          {allowedControls.includes("text45645") ?<Texttext45645   /* c95a7 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
        {allowedControls.includes("icon5645")?<Iconicon5645 /* 4d73c */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
          {allowedControls.includes("text23523") ?<Texttext23523   /* e7140 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default Groupgroup454
