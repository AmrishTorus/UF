...
import LineChartsapi_call_over_month_linechart  from "./LineChartsapi_call_over_month_linechart";
...
const Groupapi_call_over_month_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const {api_call_over_month_linechartcc886, setapi_call_over_month_linechartcc886}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Design:AFVK:v1",componentId:"6bb3bf72930743c8b2a1cb7bd4fd1676",from:"GroupApiCallOverMonthGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("api_call_over_month_linechart")){
      setapi_call_over_month_linechartcc886({...api_call_over_month_linechartcc886,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const api_call_over_month_groupd1676Ref = useRef<any>(null);
  const handleClearSearch = () => {
    api_call_over_month_groupd1676Ref.current?.setSearchParams();
    api_call_over_month_groupd1676Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(api_call_over_month_groupd1676) && Object.keys(api_call_over_month_groupd1676)?.length>0)
      {
        setapi_call_over_month_groupd1676({})
      }
    }else 
      prevRefreshRef.current= true
  }, [api_call_over_month_groupd1676Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("api_call_over_month_linechart") ?<LineChartsapi_call_over_month_linechart /* cc886 */ encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default Groupapi_call_over_month_group
