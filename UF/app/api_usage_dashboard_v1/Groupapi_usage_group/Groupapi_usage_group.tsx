...
import Textapi_usage_overview  from "./Textapi_usage_overview";
...
const Groupapi_usage_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const {api_usage_overviewecc9e, setapi_usage_overviewecc9e}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Design:AFVK:v1",componentId:"add0bd981a004738b56543151c0868b4",from:"GroupApiUsageGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("api_usage_overview")){
      setapi_usage_overviewecc9e({...api_usage_overviewecc9e,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const api_usage_group868b4Ref = useRef<any>(null);
  const handleClearSearch = () => {
    api_usage_group868b4Ref.current?.setSearchParams();
    api_usage_group868b4Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(api_usage_group868b4) && Object.keys(api_usage_group868b4)?.length>0)
      {
        setapi_usage_group868b4({})
      }
    }else 
      prevRefreshRef.current= true
  }, [api_usage_group868b4Props?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-2xl !bg-white ${isDark ? 'text-white' : 'text-black'}`}
    >
          {allowedControls.includes("api_usage_overview") ?<Textapi_usage_overview   /* ecc9e */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default Groupapi_usage_group
