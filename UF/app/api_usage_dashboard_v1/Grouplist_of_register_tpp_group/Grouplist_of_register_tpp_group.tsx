...
const Grouplist_of_register_tpp_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Design:AFVK:v1",componentId:"e5ace6874e2f43d493c10954d23be9d5",from:"GroupListOfRegisterTppGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("connected_application")){
      setconnected_application19ab2({...connected_application19ab2,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const list_of_register_tpp_groupbe9d5Ref = useRef<any>(null);
  const handleClearSearch = () => {
    list_of_register_tpp_groupbe9d5Ref.current?.setSearchParams();
    list_of_register_tpp_groupbe9d5Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(list_of_register_tpp_groupbe9d5) && Object.keys(list_of_register_tpp_groupbe9d5)?.length>0)
      {
        setlist_of_register_tpp_groupbe9d5({})
      }
    }else 
      prevRefreshRef.current= true
  }, [list_of_register_tpp_groupbe9d5Props?.refresh,token])
...
export default Grouplist_of_register_tpp_group
