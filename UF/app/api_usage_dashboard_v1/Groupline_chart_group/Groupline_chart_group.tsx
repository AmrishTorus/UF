...
import Groupapi_call_over_frequency_subscreen  from "../Groupapi_call_over_frequency_subscreen/Groupapi_call_over_frequency_subscreen";
...
import Textapi_call_hours  from "./Textapi_call_hours";
import Dropdownapi_call_hours_dropdown  from "./Dropdownapi_call_hours_dropdown";
...
const Groupline_chart_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Design:AFVK:v1",componentId:"2150f24b9f8d42f4be5c49f18b9adc5c",from:"GroupLineChartGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("api_call_hours")){
      setapi_call_hours6d062({...api_call_hours6d062,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("api_call_hours_dropdown")){
      setapi_call_hours_dropdown5f803({...api_call_hours_dropdown5f803,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("api_call_over_frequency_subscreen")){
      setapi_call_over_frequency_subscreenb8acc({...api_call_over_frequency_subscreenb8acc,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const line_chart_groupadc5cRef = useRef<any>(null);
  const handleClearSearch = () => {
    line_chart_groupadc5cRef.current?.setSearchParams();
    line_chart_groupadc5cRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(line_chart_groupadc5c) && Object.keys(line_chart_groupadc5c)?.length>0)
      {
        setline_chart_groupadc5c({})
      }
    }else 
      prevRefreshRef.current= true
  }, [line_chart_groupadc5cProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("api_call_over_frequency_subscreen")  &&<Groupapi_call_over_frequency_subscreen  
          ...
          {allowedControls.includes("api_call_hours") ?<Textapi_call_hours   /* 6d062 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
        {allowedControls.includes("api_call_hours_dropdown") ?<Dropdownapi_call_hours_dropdown   /* 5f803 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}
