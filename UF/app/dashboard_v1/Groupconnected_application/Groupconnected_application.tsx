...
import Tableconnected_application  from './Tableconnected_application';  
...
const Groupconnected_application = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Screen:AFVK:v1",componentId:"6d310786445b4ddfa4cad37d26517a5d",from:"GroupConnectedApplication",isTable : true,accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("app_name")){
      setapp_name800b4({...app_name800b4,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("tppname")){
      settppname5329d({...tppname5329d,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("type")){
      settypeb4599({...typeb4599,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("status_value")){
      setstatus_value35b5d({...status_value35b5d,isDisabled:true});
    }
  //////////////
  }
  ...
  const connected_application17a5dRef = useRef<any>(null);
  const handleClearSearch = () => {
    connected_application17a5dRef.current?.setSearchParams();
    connected_application17a5dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(connected_application17a5d) && Object.keys(connected_application17a5d)?.length>0)
      {
        setconnected_application17a5d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [connected_application17a5dProps?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tableconnected_application headerButtonsRenders={renderBUttons}
          headerPosition='top'
          headerText="Connected Application"
        lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={connected_application17a5dRef} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData}/>}
      </div>
      </div>
    </div>
 )
...
export default Groupconnected_application
