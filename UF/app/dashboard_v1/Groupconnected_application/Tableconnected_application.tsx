...
let defaultColumns = [
  {
    "id": "app_name",
    "nodeid": "d6bc94b018424afb8edd42db448800b4",
    "name": "APP Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "tppname",
    "nodeid": "f9ae217410b04862b840e69bb525329d",
    "name": "APP Provider",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "type",
    "nodeid": "a421fc7eb1f64f44ad7b055b476b4599",
    "name": "Type",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "status_value",
    "nodeid": "113e536a5bb14dc88422ece379335b5d",
    "name": "Status",
    "meta": {
      "sort": true
    },
...
const Tableconnected_application = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,primaryTableData, setPrimaryTableData,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData }: any)=>{
 ...
 /////////////
  ...
  const {app_name800b4, setapp_name800b4}= useContext(TotalContext) as TotalContextProps  
  const {tppname5329d, settppname5329d}= useContext(TotalContext) as TotalContextProps  
  const {typeb4599, settypeb4599}= useContext(TotalContext) as TotalContextProps  
  const {status_value35b5d, setstatus_value35b5d}= useContext(TotalContext) as TotalContextProps  
  //////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        componentId: '6d310786445b4ddfa4cad37d26517a5d',
        isTable: true,
        from :"Table",
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setconnected_application17a5dProps((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setconnected_application17a5dProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '6d310786445b4ddfa4cad37d26517a5d',
                elementId: '6d310786445b4ddfa4cad37d26517a5d'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( connected_application17a5dProps?.presetValues&&Object.keys(connected_application17a5dProps?.presetValues).length > 0) {
          filtertedData = [connected_application17a5dProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setconnected_application17a5d(uf_paginationDataFilter.data||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(connected_application17a5dProps.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [connected_application17a5dProps.filterProps])
  ...
  useEffect(() => {
    if (connected_application17a5dProps?.clearData === true) {
      setconnected_application17a5d([]);
      setAllDataObject([]);
      setAllData([]);
      setSelectedPaginationData([]);
      setLockedData((pre:any) => ({...pre, data:[]}));
      setconnected_application17a5dProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [connected_application17a5dProps?.clearData])
  useEffect(() => {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [connected_application17a5dProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = connected_application17a5dProps?.selectedIds[connected_application17a5dProps?.selectedIds?.length-1]
    if(Array.isArray(connected_application17a5d) && connected_application17a5d.length>0)
    {
      let data = connected_application17a5d[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "id",
        value: data["id"],
  ...
  useEffect(() => {
    if (connected_application17a5dProps?.selectedIds?.length != 0) handlePrimaryTable()
  }, [connected_application17a5dProps?.selectedIds])
  ...
  if (connected_application17a5d?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div className=' w-full h-full flex flex-row'>
            <Table
              ...
              edgePadding={true}
              selectedIds={connected_application17a5dProps?.selectedIds}  
