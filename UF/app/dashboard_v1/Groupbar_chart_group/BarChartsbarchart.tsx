...
export default function BarChartsbarchart({ 
  ...
  const handleMapperDetails=async()=>{
    ...
    '/UF/Orchestration',
      {
        ...
        componentId: "12c05ef121134196943d952db5631635",
        controlId: "072bf68868794a3a822c0f9764d46476",
        isTable: false,
    ...
    let code:string= orchestrationData?.data?.code;
      ...
      if(Array.isArray(dfd_mongo_barchart_dfd_v1Props) && dfd_mongo_barchart_dfd_v1Props?.length > 0){
        setData(dfd_mongo_barchart_dfd_v1Props)
        setbar_chart_group31635((pre:any)=>({...pre,name:dfd_mongo_barchart_dfd_v1Props[0]?.name}));
      }
      if(Array.isArray(dfd_mongo_barchart_dfd_v1Props)){
        return
      }
    }catch(err){
      console.log(err);
  ...
  if (barchart46476?.isHidden) {
    return <></>
  }
  return (
    <div
      className="w-full h-full"
      style={{gridColumn: `1 / 25`,gridRow: `1 / 97`, gap:``, height: `100%`}}
    >
      <BarChart
        data={data}
        showCurrencySign = "₹"
        title  = {`${keyset("API Calls Over Hours")}`}
        className = ""
        fillContainer={true}
        contentAlign="left"
      />
