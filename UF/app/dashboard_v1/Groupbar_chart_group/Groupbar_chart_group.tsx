...
const Groupbar_chart_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const {barchart46476, setbarchart46476}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_Dashboard_Screen:AFVK:v1",componentId:"12c05ef121134196943d952db5631635",from:"GroupBarChartGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("barchart")){
      setbarchart46476({...barchart46476,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const bar_chart_group31635Ref = useRef<any>(null);
  const handleClearSearch = () => {
    bar_chart_group31635Ref.current?.setSearchParams();
    bar_chart_group31635Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(bar_chart_group31635) && Object.keys(bar_chart_group31635)?.length>0)
      {
        setbar_chart_group31635({})
      }
    }else 
      prevRefreshRef.current= true
  }, [bar_chart_group31635Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("barchart") ?<BarChartsbarchart /* 46476 */ encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}
