...
const Cardtotal_requests = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async():Promise<void>=>{
    ...
    try{
      ...
      setreq_groupdf5e7((pre:any)=>({...pre,total_requests:dfd_mongodb_maindashboard_dfd_v1Props[0]?.total_requests}));
  }
    }catch(err){
      console.log(err);
    }
...
useEffect(() => {
  ...
  if (total_requests06c5a?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `1 / 25`,gridRow: `1 / 48`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
      style={style}
      className="p-1 !text-3xl !text-white font-bold"   
      theme="normal"
      view="clear"
      label={keyset("Total Requests")}
      disabled= {total_requests06c5a?.isDisabled ? true : false}
      onClick={handleClick} 
      contentAlign={"center"}
      >
      {req_groupdf5e7?.total_requests?req_groupdf5e7?.total_requests:"0"}
      </Card>
    </div>
  )
}
