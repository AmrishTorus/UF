...
const Carderror = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async():Promise<void>=>{
    ...
    try{
      ...
      seterror_groupcf699((pre:any)=>({...pre,error:dfd_mongodb_maindashboard_dfd_v1Props[0]?.error}));
  }
    }catch(err){
      console.log(err);
    }
...
useEffect(() => {
  ...
  return (
    ...
    style={{gridColumn: `1 / 25`,gridRow: `1 / 48`, gap:``, height: `100%`, overflow: 'auto'}} >
      ...
      disabled= {error3d67b?.isDisabled ? true : false}
      onClick={handleClick} 
      contentAlign={"center"}
      >
      {error_groupcf699?.error?error_groupcf699?.error:"0"}
      </Card>
    </div>
  )
}
