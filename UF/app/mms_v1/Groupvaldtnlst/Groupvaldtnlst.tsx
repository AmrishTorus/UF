...
import Groupvaldtndatalst  from "../Groupvaldtndatalst/Groupvaldtndatalst";
...
const Groupvaldtnlst = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1",componentId:"46331207e62c4c3aa309cdc8fd14ad99",from:"GroupValdtnlst",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("valdtndatalst")){
      setvaldtndatalstd58f5({...valdtndatalstd58f5,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const valdtnlst4ad99Ref = useRef<any>(null);
  const handleClearSearch = () => {
    valdtnlst4ad99Ref.current?.setSearchParams();
    valdtnlst4ad99Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(valdtnlst4ad99) && Object.keys(valdtnlst4ad99)?.length>0)
      {
        setvaldtnlst4ad99({})
      }
    }else 
      prevRefreshRef.current= true
  }, [valdtnlst4ad99Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("valdtndatalst")  &&<Groupvaldtndatalst  
