...
import Groupmandateinfo  from "../Groupmandateinfo/Groupmandateinfo";
import Groupmandatelst  from "../Groupmandatelst/Groupmandatelst";
import Groupdynamic_group_btns from "../Groupdynamic_group_btns/Groupdynamic_group_btns";
...
const Groupmandatedtls = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1",componentId:"b309b712559a435bb2b678d4cf32cb9f",from:"GroupMandatedtls",accessProfile:accessProfile},{
    headers: {
  ...
  for(let i=0;i<tabOptions?.length;i++){
    ...
    {
      setmandatedtls2cb9f((pre:any)=>({...pre,mandatedtls:tabOptions[i]?.id}));
      break;
    }
  }   
  /////////////
    if(orchestrationData?.data?.readableControls.includes("mandateinfo")){
      setmandateinfo1b809({...mandateinfo1b809,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("mandatelst")){
      setmandatelstc0e2c({...mandatelstc0e2c,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("dynamic_group_btns")){
      setdynamic_group_btns3c327({...dynamic_group_btns3c327,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
      codeStates['allStates']  = allStates
      codeStates['value']  = "mandateinfo"
    ...
    const handleOnload=()=>{
     setmandatedtls2cb9f((pre:any)=>({...pre,mandatedtls:"mandateinfo"}));
  }
  const handleOnChange=async(id?:string)=>{
    ...
    setmandatedtls2cb9f((pre:any)=>({...pre,mandatedtls:id}));

  }
  const mandatedtls2cb9fRef = useRef<any>(null);
  const handleClearSearch = () => {
    mandatedtls2cb9fRef.current?.setSearchParams();
    mandatedtls2cb9fRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(mandatedtls2cb9f) && Object.keys(mandatedtls2cb9f)?.length>0)
      {
        setmandatedtls2cb9f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [mandatedtls2cb9fProps?.refresh])

let tabHeaderItems : any =[
  {
    id: '0',
    title: 'dynamic_group_btns',
     content: (
      <Groupdynamic_group_btns
  ...
  return (
    ...
    <Tabs
      ...
      defaultActiveId={mandatedtls2cb9f?.mandatedtls || "mandateinfo"}
      activeTab={mandatedtls2cb9f?.mandatedtls || "mandateinfo"}
...
export default Groupmandatedtls
