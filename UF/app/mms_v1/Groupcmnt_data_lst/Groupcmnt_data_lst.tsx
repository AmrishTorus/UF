...
import Tablecmnt_data_lst  from './Tablecmnt_data_lst';  
...
const Groupcmnt_data_lst = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1",componentId:"0c12d03bab594723862b07d3c0fe3582",from:"GroupCmntDataLst",isTable : true,accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cmnts")){
      setcmnts86495({...cmnts86495,isDisabled:true});
    }
  //////////////
  }
  ...
  const cmnt_data_lste3582Ref = useRef<any>(null);
  const handleClearSearch = () => {
    cmnt_data_lste3582Ref.current?.setSearchParams();
    cmnt_data_lste3582Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(cmnt_data_lste3582) && Object.keys(cmnt_data_lste3582)?.length>0)
      {
        setcmnt_data_lste3582({})
      }
    }else 
      prevRefreshRef.current= true
  }, [cmnt_data_lste3582Props?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tablecmnt_data_lst headerButtonsRenders={renderBUttons}
        lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={cmnt_data_lste3582Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData}/>}
      </div>
      </div>
    </div>
 )
...
export default Groupcmnt_data_lst
