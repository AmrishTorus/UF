...
import Groupcmntdatalst  from "../Groupcmntdatalst/Groupcmntdatalst";
...
const Groupcmntlst = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1",componentId:"7c29bd5469cf42cd92947fa2e510f1ad",from:"GroupCmntlst",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cmntdatalst")){
      setcmntdatalste4cdc({...cmntdatalste4cdc,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const cmntlst0f1adRef = useRef<any>(null);
  const handleClearSearch = () => {
    cmntlst0f1adRef.current?.setSearchParams();
    cmntlst0f1adRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(cmntlst0f1ad) && Object.keys(cmntlst0f1ad)?.length>0)
      {
        setcmntlst0f1ad({})
      }
    }else 
      prevRefreshRef.current= true
  }, [cmntlst0f1adProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("cmntdatalst")  &&<Groupcmntdatalst  
