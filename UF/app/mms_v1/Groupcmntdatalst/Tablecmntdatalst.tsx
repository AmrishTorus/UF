...
let defaultColumns = [
  {
    "id": "cmnts",
    "nodeid": "6a79ef653ea6407db99468492a82d060",
    "name": "Comments",
    "meta": {
      "sort": true
    },
...
const Tablecmntdatalst = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,primaryTableData, setPrimaryTableData,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData }: any)=>{
 ...
 /////////////
  ...
  const {cmnts2d060, setcmnts2d060}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        componentId: '883c4ed877b74a21932bebd37c1e4cdc',
        isTable: true,
        from :"Table",
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setcmntdatalste4cdcProps((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setcmntdatalste4cdcProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '883c4ed877b74a21932bebd37c1e4cdc',
                elementId: '883c4ed877b74a21932bebd37c1e4cdc'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( cmntdatalste4cdcProps?.presetValues&&Object.keys(cmntdatalste4cdcProps?.presetValues).length > 0) {
          filtertedData = [cmntdatalste4cdcProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setcmntdatalste4cdc(uf_paginationDataFilter.data||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(cmntdatalste4cdcProps.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [cmntdatalste4cdcProps.filterProps])
  ...
  useEffect(() => {
    if (cmntdatalste4cdcProps?.clearData === true) {
      setcmntdatalste4cdc([]);
      setAllDataObject([]);
      setAllData([]);
      setSelectedPaginationData([]);
      setLockedData((pre:any) => ({...pre, data:[]}));
      setcmntdatalste4cdcProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [cmntdatalste4cdcProps?.clearData])
  useEffect(() => {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [cmntdatalste4cdcProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = cmntdatalste4cdcProps?.selectedIds[cmntdatalste4cdcProps?.selectedIds?.length-1]
    if(Array.isArray(cmntdatalste4cdc) && cmntdatalste4cdc.length>0)
    {
      let data = cmntdatalste4cdc[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "",
        value: data[""],
  ...
  useEffect(() => {
    if (cmntdatalste4cdcProps?.selectedIds?.length != 0) handlePrimaryTable()
  }, [cmntdatalste4cdcProps?.selectedIds])
    ...
    <div className='w-full h-full'>
          <div className=' w-full h-full flex flex-row'>
            <Table
              ...
              edgePadding={true}
              selectedIds={cmntdatalste4cdcProps?.selectedIds}  
...
export default Tablecmntdatalst
