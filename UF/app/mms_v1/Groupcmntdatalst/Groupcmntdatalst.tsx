...
import Tablecmntdatalst  from './Tablecmntdatalst';  
...
const Groupcmntdatalst = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1",componentId:"883c4ed877b74a21932bebd37c1e4cdc",from:"GroupCmntdatalst",isTable : true,accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cmnts")){
      setcmnts2d060({...cmnts2d060,isDisabled:true});
    }
  //////////////
  }
  ...
  const cmntdatalste4cdcRef = useRef<any>(null);
  const handleClearSearch = () => {
    cmntdatalste4cdcRef.current?.setSearchParams();
    cmntdatalste4cdcRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(cmntdatalste4cdc) && Object.keys(cmntdatalste4cdc)?.length>0)
      {
        setcmntdatalste4cdc({})
      }
    }else 
      prevRefreshRef.current= true
  }, [cmntdatalste4cdcProps?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tablecmntdatalst headerButtonsRenders={renderBUttons}
        lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={cmntdatalste4cdcRef} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData}/>}
      </div>
      </div>
    </div>
 )
...
export default Groupcmntdatalst
