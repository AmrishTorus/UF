...
import Groupmnssubgroup  from "../Groupmnssubgroup/Groupmnssubgroup";
import Groupmndate_basic_sub_screen  from "../Groupmndate_basic_sub_screen/Groupmndate_basic_sub_screen";
...
import Groupmandate_data_tab  from "../Groupmandate_data_tab/Groupmandate_data_tab";
...
const Groupmandateinfo = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1",componentId:"2d234983aea64a7b8a664058b531b809",from:"GroupMandateinfo",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("mnssubgroup")){
      setmnssubgroup3df12({...mnssubgroup3df12,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("mndate_basic_sub_screen")){
      setmndate_basic_sub_screenc9573({...mndate_basic_sub_screenc9573,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("mandate_data_tab")){
      setmandate_data_tabb1bce({...mandate_data_tabb1bce,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const mandateinfo1b809Ref = useRef<any>(null);
  const handleClearSearch = () => {
    mandateinfo1b809Ref.current?.setSearchParams();
    mandateinfo1b809Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(mandateinfo1b809) && Object.keys(mandateinfo1b809)?.length>0)
      {
        setmandateinfo1b809({})
      }
    }else 
      prevRefreshRef.current= true
  }, [mandateinfo1b809Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("mnssubgroup")  &&<Groupmnssubgroup  
        ...
        {allowedComponent.includes("mndate_basic_sub_screen")  &&<Groupmndate_basic_sub_screen  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          primaryTableData={primaryTableData}
          setPrimaryTableData={setPrimaryTableData}
          ...
          paginationDetails={paginationDetails}
          setIsProcessing={setIsProcessing}        />}
        {allowedComponent.includes("mandate_data_tab")  &&<Groupmandate_data_tab  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          primaryTableData={primaryTableData}
          setPrimaryTableData={setPrimaryTableData}
...
export default Groupmandateinfo
