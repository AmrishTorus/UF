...
import Groupcmnt_data_lst  from "../Groupcmnt_data_lst/Groupcmnt_data_lst";
...
const Groupcomment_lst = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1",componentId:"d20f87b18d234212aeb866acead2b2ca",from:"GroupCommentLst",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cmnt_data_lst")){
      setcmnt_data_lste3582({...cmnt_data_lste3582,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const comment_lst2b2caRef = useRef<any>(null);
  const handleClearSearch = () => {
    comment_lst2b2caRef.current?.setSearchParams();
    comment_lst2b2caRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(comment_lst2b2ca) && Object.keys(comment_lst2b2ca)?.length>0)
      {
        setcomment_lst2b2ca({})
      }
    }else 
      prevRefreshRef.current= true
  }, [comment_lst2b2caProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("cmnt_data_lst")  &&<Groupcmnt_data_lst  
...
export default Groupcomment_lst
