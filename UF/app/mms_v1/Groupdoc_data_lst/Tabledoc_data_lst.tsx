...
let defaultColumns = [
  {
    "id": "doc_name",
    "nodeid": "0e78711a6c544615b521e696fc76a955",
    "name": "File Name",
    "meta": {
      "sort": true
    },
...
const Tabledoc_data_lst = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,primaryTableData, setPrimaryTableData,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData }: any)=>{
 ...
 /////////////
  ...
  const {doc_name6a955, setdoc_name6a955}= useContext(TotalContext) as TotalContextProps  
  const {action55349, setaction55349}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        componentId: 'bb11e327ece246e9ac30eb202011fd5c',
        isTable: true,
        from :"Table",
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setdoc_data_lst1fd5cProps((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setdoc_data_lst1fd5cProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphsts_uuid"]:doc_data_lst1fd5cProps?.presetValues?.vgphsts_uuid}
 
    let dstKey=dfKey?.dfKey
    dstKey=dstKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    try {
    if(!doc_data_lst1fd5cProps?.presetValues || Object.keys(doc_data_lst1fd5cProps?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: 'bb11e327ece246e9ac30eb202011fd5c',
                elementId: 'bb11e327ece246e9ac30eb202011fd5c'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setdoc_data_lst1fd5c(uf_paginationDataFilter.data||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(doc_data_lst1fd5c))
    {
      GetTableDetails()
    }
  }, [doc_data_lst1fd5c])
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(doc_data_lst1fd5cProps.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [doc_data_lst1fd5cProps.filterProps])
  ...
  useEffect(() => {
    if (doc_data_lst1fd5cProps?.clearData === true) {
      setdoc_data_lst1fd5c([]);
      setAllDataObject([]);
      setAllData([]);
      setSelectedPaginationData([]);
      setLockedData((pre:any) => ({...pre, data:[]}));
      setdoc_data_lst1fd5cProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [doc_data_lst1fd5cProps?.clearData])
  useEffect(() => {
    setdoc_data_lst1fd5c([])
    setdoc_data_lst1fd5cProps((pre:any)=>({...pre, selectedIds:[]}))
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [doc_data_lst1fd5cProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = doc_data_lst1fd5cProps?.selectedIds[doc_data_lst1fd5cProps?.selectedIds?.length-1]
    if(Array.isArray(doc_data_lst1fd5c) && doc_data_lst1fd5c.length>0)
    {
      let data = doc_data_lst1fd5c[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphstds_id",
        value: data["vgphstds_id"],
        parentData: data
      })
    }
  }
  ...
  useEffect(() => {
    if (doc_data_lst1fd5cProps?.selectedIds?.length != 0) handlePrimaryTable()
  }, [doc_data_lst1fd5cProps?.selectedIds])
    ...
    <div className='w-full h-full'>
          <div className=' w-full h-full flex flex-row'>
            <Table
              ...
              edgePadding={true}
              selectedIds={doc_data_lst1fd5cProps?.selectedIds}  
...
export default Tabledoc_data_lst
