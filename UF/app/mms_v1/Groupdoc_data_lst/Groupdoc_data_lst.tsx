...
import Tabledoc_data_lst  from './Tabledoc_data_lst';  
...
const Groupdoc_data_lst = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1",componentId:"bb11e327ece246e9ac30eb202011fd5c",from:"GroupDocDataLst",isTable : true,accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("doc_name")){
      setdoc_name6a955({...doc_name6a955,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("action")){
      setaction55349({...action55349,isDisabled:true});
    }
  //////////////
  }
  ...
  const doc_data_lst1fd5cRef = useRef<any>(null);
  const handleClearSearch = () => {
    doc_data_lst1fd5cRef.current?.setSearchParams();
    doc_data_lst1fd5cRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(doc_data_lst1fd5c) && Object.keys(doc_data_lst1fd5c)?.length>0)
      {
        setdoc_data_lst1fd5c({})
      }
    }else 
      prevRefreshRef.current= true
  }, [doc_data_lst1fd5cProps?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tabledoc_data_lst headerButtonsRenders={renderBUttons}
        lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={doc_data_lst1fd5cRef} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData}/>}
      </div>
      </div>
    </div>
 )
...
export default Groupdoc_data_lst
