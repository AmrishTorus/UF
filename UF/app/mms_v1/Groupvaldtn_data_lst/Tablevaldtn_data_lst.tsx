...
let defaultColumns = [
  {
    "id": "vld_code",
    "nodeid": "f187f34d5c154be39178c95691a5df31",
    "name": "Vld Code",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "vld_reason",
    "nodeid": "0639698b84c64eceb3cc99631790ec55",
    "name": "Vld Reason",
    "meta": {
      "sort": true
    },
...
const Tablevaldtn_data_lst = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,primaryTableData, setPrimaryTableData,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData }: any)=>{
 ...
 /////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        componentId: '0cd3582385e945bd94ae4cf9864378bc',
        isTable: true,
        from :"Table",
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setvaldtn_data_lst378bcProps((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setvaldtn_data_lst378bcProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphsts_uuid"]:valdtn_data_lst378bcProps?.presetValues?.vgphsts_uuid}
 
    let dstKey=dfKey?.dfKey
    dstKey=dstKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    try {
    if(!valdtn_data_lst378bcProps?.presetValues || Object.keys(valdtn_data_lst378bcProps?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '0cd3582385e945bd94ae4cf9864378bc',
                elementId: '0cd3582385e945bd94ae4cf9864378bc'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setvaldtn_data_lst378bc(uf_paginationDataFilter.data||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(valdtn_data_lst378bc))
    {
      GetTableDetails()
    }
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(valdtn_data_lst378bcProps.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [valdtn_data_lst378bcProps.filterProps])
  ...
  useEffect(() => {
    if (valdtn_data_lst378bcProps?.clearData === true) {
      setvaldtn_data_lst378bc([]);
      setAllDataObject([]);
      setAllData([]);
      setSelectedPaginationData([]);
      setLockedData((pre:any) => ({...pre, data:[]}));
      setvaldtn_data_lst378bcProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [valdtn_data_lst378bcProps?.clearData])
  useEffect(() => {
    setvaldtn_data_lst378bc([])
    setvaldtn_data_lst378bcProps((pre:any)=>({...pre, selectedIds:[]}))
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [valdtn_data_lst378bcProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = valdtn_data_lst378bcProps?.selectedIds[valdtn_data_lst378bcProps?.selectedIds?.length-1]
    if(Array.isArray(valdtn_data_lst378bc) && valdtn_data_lst378bc.length>0)
    {
      let data = valdtn_data_lst378bc[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphtelm_id",
  ...
  useEffect(() => {
    if (valdtn_data_lst378bcProps?.selectedIds?.length != 0) handlePrimaryTable()
  }, [valdtn_data_lst378bcProps?.selectedIds])
  ...
  if (valdtn_data_lst378bc?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div className=' w-full h-full flex flex-row'>
            <Table
              ...
              edgePadding={true}
              selectedIds={valdtn_data_lst378bcProps?.selectedIds}  
...
export default Tablevaldtn_data_lst
