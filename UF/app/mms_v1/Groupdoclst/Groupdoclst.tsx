...
import Groupdocdatalst  from "../Groupdocdatalst/Groupdocdatalst";
...
const Groupdoclst = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1",componentId:"b25b9c110d004d93b0c61f92b2542d30",from:"GroupDoclst",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("docdatalst")){
      setdocdatalst620a8({...docdatalst620a8,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const doclst42d30Ref = useRef<any>(null);
  const handleClearSearch = () => {
    doclst42d30Ref.current?.setSearchParams();
    doclst42d30Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(doclst42d30) && Object.keys(doclst42d30)?.length>0)
      {
        setdoclst42d30({})
      }
    }else 
      prevRefreshRef.current= true
  }, [doclst42d30Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("docdatalst")  &&<Groupdocdatalst  
