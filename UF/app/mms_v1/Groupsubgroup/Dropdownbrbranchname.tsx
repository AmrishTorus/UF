...
const Dropdownbrbranchname = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}: any) => {
 ...
 /////////////
  ...
  const [brbranchnameOptions, setbrbranchnameOptions] = useState<string[]>([]);
  let category : string
  ...
  const handleMapperValue = async()=>{
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        controlId: "5412727175ab437f8273f04359d951ce",
        isTable: false,
  ...
  const getDropdownData = async(value?:any, page: number = 1)=>{
    ...
    let mapperColumn: string =  `branch_name`
    let mapperText: string =  `branch_name`
  ...
  try{
    ...
    if(!bindtranValue){
      ...
      setsubgroup7f52e((prev: any) => ({ ...prev, brbranchname: getMapperDetails }))
    }
    if(!value){
    let temp:any[] = getMapperDetails.filter((item:any, index:any) => getMapperDetails.indexOf(item) === index)
    temp = temp.filter((ele:any)=>ele)
    if (page === 1) {
      setbrbranchnameOptions(temp);
    } else {
      setbrbranchnameOptions((prev: any[]) => {
  ...
  const handlechange = async(value: any) => {
    ...
    if(value.length>0){
      setsubgroup7f52e((prev: any) => ({ ...prev, brbranchname: getMapperDetailsBindValues[value],brbranchname951ce: value }))
      getDropdownData(value)
      setIsRequredData(false)
    }else{
      let temp:any = subgroup7f52e
      delete temp.brbranchname
      delete temp.brbranchname951ce
      setsubgroup7f52e(temp)
      getDropdownData()
       setIsRequredData(true)
    }
     setError('')
    setValidate((pre:any)=>({...pre,Mms_Mandate_Info_v1:{...pre?.Mms_Mandate_Info_v1,brbranchname:undefined}}));
  };

    const fetchDropdownData = async()=>{
    if(subgroup7f52e.brbranchname){
      if(Array.isArray(dfd_combo_subscreen_db_v1Props)){
        if(dfd_combo_subscreen_db_v1Props?.find((item: any) => item.branch_name === subgroup7f52e.brbranchname)){
          setdropdownValue([dfd_combo_subscreen_db_v1Props?.find((item: any) => item.branch_name === subgroup7f52e.brbranchname)?.brbranchname])
        }else{
          setdropdownValue([subgroup7f52e.brbranchname])
        }
      }else{
        let dstKey:string = dfd_combo_subscreen_db_v1Props.dstKey;
        const api_paginationData:any = await AxiosService.post(
        '/UF/pagination',
        {
          ...
          page:currentPage,
          count:PAGE_SIZE,
          searchFilter:{branch_name:subgroup7f52e.brbranchname}
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if(api_paginationData?.data?.records?.find((item: any) => item.branch_name === subgroup7f52e.brbranchname)){
        setdropdownValue([api_paginationData?.data?.records?.find((item: any) => item.branch_name === subgroup7f52e.brbranchname)?.branch_name ])
      }else{
        setdropdownValue([subgroup7f52e.brbranchname])
      }   
      }
  ...
  useEffect(() => {
    ...
    fetchDropdownData();
  },[subgroup7f52e.brbranchname, isDropdownDataReady])

  useEffect(() => {
    if(Array.isArray(dfd_combo_subscreen_db_v1Props) && dfd_combo_subscreen_db_v1Props?.length == 1){
    // setsubgroup7f52e((pre:any)=>({...pre,brbranchname:dfd_combo_subscreen_db_v1Props[0]?.brbranchname}))
    }
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setsubgroup7f52e((prev: any) => ({ ...prev, brbranchname: getMapperDetailsBindValues[value]}))
         setIsRequredData(false)
    } else {
       setsubgroup7f52e((prev: any) => ({ ...prev, brbranchname: ''}))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,Mms_Mandate_Info_v1:{...pre?.Mms_Mandate_Info_v1,brbranchname:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
      if(subgroup7f52e?.brbranchname == "" || subgroup7f52e?.brbranchname == undefined){
      subgroup7f52e.brbranchname = "";
      const validate:any = v.safeParse(schema, subgroup7f52e?.brbranchname);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,Mms_Mandate_Info_v1:{...pre?.Mms_Mandate_Info_v1,brbranchname:"invalid"}}));
        }
    }else if(subgroup7f52e?.brbranchname !== ""){
    const validate:any = v.safeParse(schema, subgroup7f52e?.brbranchname);
    if(!validate.success){
      setError(validate?.issues[0]?.message);
      setValidate((pre:any)=>({...pre,Mms_Mandate_Info_v1:{...pre?.Mms_Mandate_Info_v1,brbranchname:"invalid"}}));
  ...
  useEffect(() => {
    if(initialCount!=0)
     setsubgroup7f52e((pre:any)=>({...pre,brbranchname:""}))
    else
      setInitialCount(1)
  },[brbranchname951ce?.refresh])

  if (brbranchname951ce?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown   
        ...
        disabled= {brbranchname951ce?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        static={true}
        staticProps={brbranchnameOptions}
        onLoadMore={loadMore}
        ...
        value={subgroup7f52e?.brbranchname951ce ? [subgroup7f52e?.brbranchname951ce] : (subgroup7f52e?.brbranchname ? dropdownValue : [])}
        validationState={validate?.Mms_Mandate_Info_v1?.brbranchname ? "invalid" : undefined}
        errorMessage={error}
        />
    </div>
  );
...
export default Dropdownbrbranchname;
