...
const Dropdownfrequency = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
  if(subgroup7f52e?.frequency=="" || subgroup7f52e?.frequency==undefined || subgroup7f52e?.frequency==null ){
    setSelectedItem("");
  }
  },[subgroup7f52e?.frequency])
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "05ca00f68fb2429c91a5cc6546c80676",
          isTable: false,
          accessProfile:accessProfile,
          from:"dropdownFrequency"
        },
        {
          headers: {
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setsubgroup7f52e((prev: any) => ({ ...prev, frequency: value}))
         setIsRequredData(false)
    } else {
       setsubgroup7f52e((prev: any) => ({ ...prev, frequency: ''}))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,Mms_Mandate_Info_v1:{...pre?.Mms_Mandate_Info_v1,frequency:undefined}}));
  ...
  const handleBlur = async () => {
    //validation
      if(subgroup7f52e?.frequency == "" || subgroup7f52e?.frequency == undefined){
      subgroup7f52e.frequency = "";
      const validate:any = v.safeParse(schema, subgroup7f52e?.frequency);
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,Mms_Mandate_Info_v1:{...pre?.Mms_Mandate_Info_v1,frequency:"invalid"}}));
        }
    }else if(subgroup7f52e?.frequency !== ""){
    const validate:any = v.safeParse(schema, subgroup7f52e?.frequency);
    if(!validate.success){
      setError(validate?.issues[0]?.message);
      setValidate((pre:any)=>({...pre,Mms_Mandate_Info_v1:{...pre?.Mms_Mandate_Info_v1,frequency:"invalid"}}));
  ...
  useEffect(() => {
    if(initialCount!=0)
     setsubgroup7f52e((pre:any)=>({...pre,frequency:""}))
    else
      setInitialCount(1)
  },[frequency80676?.refresh])

  if (frequency80676?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown
        ...
        placeholder={keyset("Frequency")} 
        filterable={true}
        hasClear={true}
        static={true}
        staticProps={items}
        disabled= {frequency80676?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={subgroup7f52e?.frequency ?subgroup7f52e?.frequency: []}
        onChange={handleClick} 
        validationState={validate?.Mms_Mandate_Info_v1?.frequency ? "invalid" : undefined}
        errorMessage={error}
      /> 
    </div>
  );
