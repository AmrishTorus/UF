...
const TextInputdr_sort_code = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:V001:AFGK:MMS:AFK:subscreen_db:AFVK:v1|0a399637910841a79c8f605d784233ee|properties.dr_sort_code"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1|4ec956e4599e4a4ab60a0db92557f52e|6159a09c49df43b3993ac7eea6b26ce5"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'dr_sort_code',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,Mms_Mandate_Info_v1:{...pre?.Mms_Mandate_Info_v1,dr_sort_code:undefined}}));
    ...
    setsubgroup7f52e((prev: any) => ({ ...prev, dr_sort_code: +e.target.value }));
    }
    else{
    setsubgroup7f52e((prev: any) => ({ ...prev, dr_sort_code: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['overallgroup']  = {...overallgroup00e53,dr_sort_code:newInputValue},
      codeStates['setoverallgroup'] = setoverallgroup00e53,
      codeStates['mnssubgroup']  = {...mnssubgroup3df12,dr_sort_code:newInputValue},
      codeStates['setmnssubgroup'] = setmnssubgroup3df12,
      codeStates['ct005_af_uf_ufws_v001_mms_mandate_subscreen2_v1']  = {...ct005_af_uf_ufws_v001_mms_mandate_subscreen2_v1a54b9,dr_sort_code:newInputValue},
      codeStates['setct005_af_uf_ufws_v001_mms_mandate_subscreen2_v1'] = setct005_af_uf_ufws_v001_mms_mandate_subscreen2_v1a54b9,
      codeStates['mndate_common_info']  = {...mndate_common_info3fb9d,dr_sort_code:newInputValue},
      codeStates['setmndate_common_info'] = setmndate_common_info3fb9d,
      codeStates['mndate_basic_sub_screen']  = {...mndate_basic_sub_screenc9573,dr_sort_code:newInputValue},
      codeStates['setmndate_basic_sub_screen'] = setmndate_basic_sub_screenc9573,
      codeStates['ct005_af_uf_ufws_v001_mms_mandate_subscreen_v1']  = {...ct005_af_uf_ufws_v001_mms_mandate_subscreen_v12764c,dr_sort_code:newInputValue},
      codeStates['setct005_af_uf_ufws_v001_mms_mandate_subscreen_v1'] = setct005_af_uf_ufws_v001_mms_mandate_subscreen_v12764c,
      codeStates['subgroup']  = {...subgroup7f52e,dr_sort_code:newInputValue},
      codeStates['setsubgroup'] = setsubgroup7f52e,
      codeStates['doc_data_lst']  = {...doc_data_lst1fd5c,dr_sort_code:newInputValue},
      codeStates['setdoc_data_lst'] = setdoc_data_lst1fd5c,
      codeStates['valdtn_data_lst']  = {...valdtn_data_lst378bc,dr_sort_code:newInputValue},
      codeStates['setvaldtn_data_lst'] = setvaldtn_data_lst378bc,
      codeStates['cmnt_data_lst']  = {...cmnt_data_lste3582,dr_sort_code:newInputValue},
      codeStates['setcmnt_data_lst'] = setcmnt_data_lste3582,
      codeStates['mandatedatalst']  = {...mandatedatalst46c27,dr_sort_code:newInputValue},
      codeStates['setmandatedatalst'] = setmandatedatalst46c27,
      codeStates['docdatalst']  = {...docdatalst620a8,dr_sort_code:newInputValue},
      codeStates['setdocdatalst'] = setdocdatalst620a8,
      codeStates['valdtndatalst']  = {...valdtndatalstd58f5,dr_sort_code:newInputValue},
      codeStates['setvaldtndatalst'] = setvaldtndatalstd58f5,
      codeStates['cmntdatalst']  = {...cmntdatalste4cdc,dr_sort_code:newInputValue},
      codeStates['setcmntdatalst'] = setcmntdatalste4cdc,
      codeStates['dynamic_group_btns']  = {...dynamic_group_btns3c327,dr_sort_code:newInputValue},
  ...
  const handleBlur=async () => {
      let validate:any
      if(subgroup7f52e?.dr_sort_code == "" || subgroup7f52e?.dr_sort_code == undefined){
      subgroup7f52e.dr_sort_code = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, subgroup7f52e?.dr_sort_code);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,Mms_Mandate_Info_v1:{...pre?.Mms_Mandate_Info_v1,dr_sort_code:"invalid"}}));
        }
    }else if(subgroup7f52e?.dr_sort_code !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +subgroup7f52e?.dr_sort_code);
        }
        else{
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "6159a09c49df43b3993ac7eea6b26ce5",
          isTable: false,
          from:"TextInputdr_sort_code",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'dr_sort_code', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'dr_sort_code',type:'text'};
      //   type={
      //     name:'dr_sort_code',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.dr_sort_code.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.dr_sort_code.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.dr_sort_code.type
      //   }
      ...
      //   type={
      //     name:'dr_sort_code',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.dr_sort_code.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.dr_sort_code.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.dr_sort_code.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  if(dfd_subscreen_db_v1Props?.setSearchFilters && dfd_subscreen_db_v1Props?.data)
  {
    if(Array.isArray(dfd_subscreen_db_v1Props.data) && dfd_subscreen_db_v1Props.data.length > 0){
      setsubgroup7f52e((pre:any)=>({...pre,dr_sort_code:dfd_subscreen_db_v1Props.data[0]?.dr_sort_code}));
    }
  }
  },[dfd_subscreen_db_v1Props?.setSearchFilters])
  if (dr_sort_code26ce5?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `14 / 19`,gridRow: `56 / 76`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value= {dynamicStateandType.type === "number" ? formatNumberWithCommas(subgroup7f52e?.dr_sort_code) || "" : subgroup7f52e?.dr_sort_code|| ""}
         disabled= {dr_sort_code26ce5?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Dr Sort Code"
      errorMessage={error}
        validationState={validate?.Mms_Mandate_Info_v1?.dr_sort_code ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputdr_sort_code
