...
const TextInputceiling_amnt = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:V001:AFGK:MMS:AFK:subscreen_db:AFVK:v1|0a399637910841a79c8f605d784233ee|properties.celing_amount"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1|4ec956e4599e4a4ab60a0db92557f52e|d3131fb26fbd4857ad44bfb3908c6b28"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'celing_amount',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,Mms_Mandate_Info_v1:{...pre?.Mms_Mandate_Info_v1,celing_amount:undefined}}));
    ...
    setsubgroup7f52e((prev: any) => ({ ...prev, celing_amount: +e.target.value }));
    }
    else{
    setsubgroup7f52e((prev: any) => ({ ...prev, celing_amount: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['overallgroup']  = {...overallgroup00e53,celing_amount:newInputValue},
      codeStates['setoverallgroup'] = setoverallgroup00e53,
      codeStates['mnssubgroup']  = {...mnssubgroup3df12,celing_amount:newInputValue},
      codeStates['setmnssubgroup'] = setmnssubgroup3df12,
      codeStates['ct005_af_uf_ufws_v001_mms_mandate_subscreen2_v1']  = {...ct005_af_uf_ufws_v001_mms_mandate_subscreen2_v1a54b9,celing_amount:newInputValue},
      codeStates['setct005_af_uf_ufws_v001_mms_mandate_subscreen2_v1'] = setct005_af_uf_ufws_v001_mms_mandate_subscreen2_v1a54b9,
      codeStates['mndate_common_info']  = {...mndate_common_info3fb9d,celing_amount:newInputValue},
      codeStates['setmndate_common_info'] = setmndate_common_info3fb9d,
      codeStates['mndate_basic_sub_screen']  = {...mndate_basic_sub_screenc9573,celing_amount:newInputValue},
      codeStates['setmndate_basic_sub_screen'] = setmndate_basic_sub_screenc9573,
      codeStates['ct005_af_uf_ufws_v001_mms_mandate_subscreen_v1']  = {...ct005_af_uf_ufws_v001_mms_mandate_subscreen_v12764c,celing_amount:newInputValue},
      codeStates['setct005_af_uf_ufws_v001_mms_mandate_subscreen_v1'] = setct005_af_uf_ufws_v001_mms_mandate_subscreen_v12764c,
      codeStates['subgroup']  = {...subgroup7f52e,celing_amount:newInputValue},
      codeStates['setsubgroup'] = setsubgroup7f52e,
      codeStates['doc_data_lst']  = {...doc_data_lst1fd5c,celing_amount:newInputValue},
      codeStates['setdoc_data_lst'] = setdoc_data_lst1fd5c,
      codeStates['valdtn_data_lst']  = {...valdtn_data_lst378bc,celing_amount:newInputValue},
      codeStates['setvaldtn_data_lst'] = setvaldtn_data_lst378bc,
      codeStates['cmnt_data_lst']  = {...cmnt_data_lste3582,celing_amount:newInputValue},
      codeStates['setcmnt_data_lst'] = setcmnt_data_lste3582,
      codeStates['mandatedatalst']  = {...mandatedatalst46c27,celing_amount:newInputValue},
      codeStates['setmandatedatalst'] = setmandatedatalst46c27,
      codeStates['docdatalst']  = {...docdatalst620a8,celing_amount:newInputValue},
      codeStates['setdocdatalst'] = setdocdatalst620a8,
      codeStates['valdtndatalst']  = {...valdtndatalstd58f5,celing_amount:newInputValue},
      codeStates['setvaldtndatalst'] = setvaldtndatalstd58f5,
      codeStates['cmntdatalst']  = {...cmntdatalste4cdc,celing_amount:newInputValue},
      codeStates['setcmntdatalst'] = setcmntdatalste4cdc,
      codeStates['dynamic_group_btns']  = {...dynamic_group_btns3c327,celing_amount:newInputValue},
  ...
  const handleBlur=async () => {
      let validate:any
      if(subgroup7f52e?.celing_amount == "" || subgroup7f52e?.celing_amount == undefined){
      subgroup7f52e.celing_amount = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, subgroup7f52e?.celing_amount);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,Mms_Mandate_Info_v1:{...pre?.Mms_Mandate_Info_v1,celing_amount:"invalid"}}));
        }
    }else if(subgroup7f52e?.celing_amount !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +subgroup7f52e?.celing_amount);
        }
        else{
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "d3131fb26fbd4857ad44bfb3908c6b28",
          isTable: false,
          from:"TextInputceiling_amnt",
      ...
      //   type={
      //     name:'celing_amount',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.celing_amount.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.celing_amount.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.celing_amount.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!subgroup7f52e?.celing_amount)
      { 
  ...
  if(dfd_subscreen_db_v1Props?.setSearchFilters && dfd_subscreen_db_v1Props?.data)
  {
    if(Array.isArray(dfd_subscreen_db_v1Props.data) && dfd_subscreen_db_v1Props.data.length > 0){
      setsubgroup7f52e((pre:any)=>({...pre,celing_amount:dfd_subscreen_db_v1Props.data[0]?.celing_amount}));
    }
  }
  },[dfd_subscreen_db_v1Props?.setSearchFilters])
  if (ceiling_amntc6b28?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `20 / 25`,gridRow: `28 / 46`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value= {dynamicStateandType.type === "number" ? formatNumberWithCommas(subgroup7f52e?.celing_amount) || "" : subgroup7f52e?.celing_amount|| ""}
         disabled= {ceiling_amntc6b28?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Ceiling Amount"
      errorMessage={error}
        validationState={validate?.Mms_Mandate_Info_v1?.celing_amount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputceiling_amnt
