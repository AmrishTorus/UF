...
const Checkboxinhouse = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, string>>({name:'clr_house_type',type:"text"});
 ...
 /////////////
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "8b1b83fe0d654a0ea29c5d88c310b227",
          isTable: false,
  ...
  useEffect(()=>{
    handleMapperValue()
  },[inhouse0b227?.refresh])

  const handleChange=async(checked:boolean)=>{
    setsubgroup7f52e((prev: any) => ({ ...prev, clr_house_type: checked}));
  }
  ...
  if (inhouse0b227?.isHidden) {
    return <></>;
  }
  return (
    <div 
       style={{gridColumn: `6 / 9`,gridRow: `13 / 21`, gap:``, height: `100%`, overflow: 'auto'}} >
    <Checkbox 
      className=""
      value={subgroup7f52e?.clr_house_type||false}
      checked={subgroup7f52e?.clr_house_type||false}
      disabled= {inhouse0b227?.isDisabled ? true : false}
      content = {'Inhouse'}
...
export default Checkboxinhouse;
