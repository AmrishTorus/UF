...
const DatePickerduedate = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {
...
const handleUpdate = async(date: any) => {
   if(date == "" || date == null || date == undefined) {
    ...
    setValidate((pre:any)=>({...pre,Mms_Mandate_Info_v1:{...pre?.Mms_Mandate_Info_v1,due_date: "invalid"}}))
    setsubgroup7f52e((prev: any) => ({ ...prev, due_date: "" }))
    return;
  }
  setError('')
  setValidate((pre:any)=>({...pre,Mms_Mandate_Info_v1:{...pre?.Mms_Mandate_Info_v1,due_date:undefined}}));
  if (!date) {
    setsubgroup7f52e((prev: any) => ({ ...prev, due_date: null }));
    return;
  }
  ...
  setsubgroup7f52e((prev: any) => ({ ...prev, due_date: isoDate }))
}
...
const handleBlur=async () => {
    //validation
    if(subgroup7f52e?.due_date == "" || subgroup7f52e?.due_date == undefined){
    subgroup7f52e.due_date = "";
    const validate:any = v.safeParse(schema, subgroup7f52e?.due_date);
    if(!validate.success){
    ...
    }else if(subgroup7f52e?.due_date !== ""){
      const validate:any = v.safeParse(schema, subgroup7f52e?.due_date);  
      if(!validate.success){
        setError(validate?.issues[0]?.message);
        setValidate((pre:any)=>({...pre,Mms_Mandate_Info_v1:{...pre?.Mms_Mandate_Info_v1,due_date: "invalid"}}));
    ...
    let code:any;
    const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1",  componentId:"4ec956e4599e4a4ab60a0db92557f52e",controlId:"3ae04050a8c040c0abd82d5ca023cf41",isTable:false,accessProfile:accessProfile,from:"datePickerDue Date"},{
      headers: {
...
useEffect(()=>{
 ...
 },[duedate3cf41?.refresh])

useEffect(()=>{
  if(!subgroup7f52e?.due_date){ 
...
if (duedate3cf41?.isHidden) {
  return <></>
}
return (
  <div 
  style={{gridColumn: `14 / 19`,gridRow: `113 / 132`, gap:``, height: `100%`, overflow: 'auto'}} >
    <DatePicker
      className="!rounded-xl"
      //label={keyset("Due Date")}
      value={subgroup7f52e?.due_date}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      required={ true }
      readOnly=  {duedate3cf41?.isDisabled ? true : false}
      disabled= {duedate3cf41?.isDisabled ? true : false}
      contentAlign={"center"}
      headerPosition='top'
      headerText="Due Date"
      validationState={validate?.Mms_Mandate_Info_v1?.due_date ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
...
export default DatePickerduedate
