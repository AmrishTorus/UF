...
const TextInputoriginator_code = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:V001:AFGK:MMS:AFK:subscreen_db:AFVK:v1|0a399637910841a79c8f605d784233ee|properties.originator_code"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1|4ec956e4599e4a4ab60a0db92557f52e|669e38d7991d41dfb6d1103014ce7d60"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'originator_code',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,Mms_Mandate_Info_v1:{...pre?.Mms_Mandate_Info_v1,originator_code:undefined}}));
    ...
    setsubgroup7f52e((prev: any) => ({ ...prev, originator_code: +e.target.value }));
    }
    else{
    setsubgroup7f52e((prev: any) => ({ ...prev, originator_code: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['overallgroup']  = {...overallgroup00e53,originator_code:newInputValue},
      codeStates['setoverallgroup'] = setoverallgroup00e53,
      codeStates['mnssubgroup']  = {...mnssubgroup3df12,originator_code:newInputValue},
      codeStates['setmnssubgroup'] = setmnssubgroup3df12,
      codeStates['ct005_af_uf_ufws_v001_mms_mandate_subscreen2_v1']  = {...ct005_af_uf_ufws_v001_mms_mandate_subscreen2_v1a54b9,originator_code:newInputValue},
      codeStates['setct005_af_uf_ufws_v001_mms_mandate_subscreen2_v1'] = setct005_af_uf_ufws_v001_mms_mandate_subscreen2_v1a54b9,
      codeStates['mndate_common_info']  = {...mndate_common_info3fb9d,originator_code:newInputValue},
      codeStates['setmndate_common_info'] = setmndate_common_info3fb9d,
      codeStates['mndate_basic_sub_screen']  = {...mndate_basic_sub_screenc9573,originator_code:newInputValue},
      codeStates['setmndate_basic_sub_screen'] = setmndate_basic_sub_screenc9573,
      codeStates['ct005_af_uf_ufws_v001_mms_mandate_subscreen_v1']  = {...ct005_af_uf_ufws_v001_mms_mandate_subscreen_v12764c,originator_code:newInputValue},
      codeStates['setct005_af_uf_ufws_v001_mms_mandate_subscreen_v1'] = setct005_af_uf_ufws_v001_mms_mandate_subscreen_v12764c,
      codeStates['subgroup']  = {...subgroup7f52e,originator_code:newInputValue},
      codeStates['setsubgroup'] = setsubgroup7f52e,
      codeStates['doc_data_lst']  = {...doc_data_lst1fd5c,originator_code:newInputValue},
      codeStates['setdoc_data_lst'] = setdoc_data_lst1fd5c,
      codeStates['valdtn_data_lst']  = {...valdtn_data_lst378bc,originator_code:newInputValue},
      codeStates['setvaldtn_data_lst'] = setvaldtn_data_lst378bc,
      codeStates['cmnt_data_lst']  = {...cmnt_data_lste3582,originator_code:newInputValue},
      codeStates['setcmnt_data_lst'] = setcmnt_data_lste3582,
      codeStates['mandatedatalst']  = {...mandatedatalst46c27,originator_code:newInputValue},
      codeStates['setmandatedatalst'] = setmandatedatalst46c27,
      codeStates['docdatalst']  = {...docdatalst620a8,originator_code:newInputValue},
      codeStates['setdocdatalst'] = setdocdatalst620a8,
      codeStates['valdtndatalst']  = {...valdtndatalstd58f5,originator_code:newInputValue},
      codeStates['setvaldtndatalst'] = setvaldtndatalstd58f5,
      codeStates['cmntdatalst']  = {...cmntdatalste4cdc,originator_code:newInputValue},
      codeStates['setcmntdatalst'] = setcmntdatalste4cdc,
      codeStates['dynamic_group_btns']  = {...dynamic_group_btns3c327,originator_code:newInputValue},
  ...
  const handleBlur=async () => {
      let validate:any
      if(subgroup7f52e?.originator_code == "" || subgroup7f52e?.originator_code == undefined){
      subgroup7f52e.originator_code = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, subgroup7f52e?.originator_code);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,Mms_Mandate_Info_v1:{...pre?.Mms_Mandate_Info_v1,originator_code:"invalid"}}));
        }
    }else if(subgroup7f52e?.originator_code !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +subgroup7f52e?.originator_code);
        }
        else{
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "669e38d7991d41dfb6d1103014ce7d60",
          isTable: false,
          from:"TextInputoriginator_code",
      ...
      //   type={
      //     name:'originator_code',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.originator_code.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.originator_code.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.originator_code.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!subgroup7f52e?.originator_code)
      { 
  ...
  if(dfd_subscreen_db_v1Props?.setSearchFilters && dfd_subscreen_db_v1Props?.data)
  {
    if(Array.isArray(dfd_subscreen_db_v1Props.data) && dfd_subscreen_db_v1Props.data.length > 0){
      setsubgroup7f52e((pre:any)=>({...pre,originator_code:dfd_subscreen_db_v1Props.data[0]?.originator_code}));
    }
  }
  },[dfd_subscreen_db_v1Props?.setSearchFilters])
  if (originator_codee7d60?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `2 / 7`,gridRow: `113 / 132`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value= {dynamicStateandType.type === "number" ? formatNumberWithCommas(subgroup7f52e?.originator_code) || "" : subgroup7f52e?.originator_code|| ""}
         disabled= {originator_codee7d60?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Originator Code"
      errorMessage={error}
        validationState={validate?.Mms_Mandate_Info_v1?.originator_code ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputoriginator_code
