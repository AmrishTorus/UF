...
import Groupmandatedatalst  from "../Groupmandatedatalst/Groupmandatedatalst";
...
const Groupmandatelst = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1",componentId:"b44f0844b8c04e87af0739ee5f0c0e2c",from:"GroupMandatelst",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("mandatedatalst")){
      setmandatedatalst46c27({...mandatedatalst46c27,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("listgroup")){
      setlistgroup6f63e({...listgroup6f63e,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const mandatelstc0e2cRef = useRef<any>(null);
  const handleClearSearch = () => {
    mandatelstc0e2cRef.current?.setSearchParams();
    mandatelstc0e2cRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(mandatelstc0e2c) && Object.keys(mandatelstc0e2c)?.length>0)
      {
        setmandatelstc0e2c({})
      }
    }else 
      prevRefreshRef.current= true
  }, [mandatelstc0e2cProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("mandatedatalst")  &&<Groupmandatedatalst  
...
export default Groupmandatelst
