...
let defaultColumns = [
  {
    "id": "docname",
    "nodeid": "37cc8658a2514b8bb055b9a2a51fb324",
    "name": "File Name",
    "meta": {
      "sort": true
    },
...
const Tabledocdatalst = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,primaryTableData, setPrimaryTableData,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData }: any)=>{
 ...
 /////////////
  ...
  const {docnamefb324, setdocnamefb324}= useContext(TotalContext) as TotalContextProps  
  const {deleted70e2, setdeleted70e2}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        componentId: 'ba7a656c3d504ab59221ea7c740620a8',
        isTable: true,
        from :"Table",
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setdocdatalst620a8Props((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setdocdatalst620a8Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: 'ba7a656c3d504ab59221ea7c740620a8',
                elementId: 'ba7a656c3d504ab59221ea7c740620a8'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( docdatalst620a8Props?.presetValues&&Object.keys(docdatalst620a8Props?.presetValues).length > 0) {
          filtertedData = [docdatalst620a8Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setdocdatalst620a8(uf_paginationDataFilter.data||[])
        }
...
////////////////////////////////
  const RowAction = ({item,index,nodeName}: any) => {
    ...
    if(handleSecurity("delete") &&nodeName?.toLowerCase()=="buttondelete"){
      return (
        <React.Fragment>
        <Buttondelete mainData={filteredData} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData}/>
        </React.Fragment>
      )
    }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(docdatalst620a8Props.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [docdatalst620a8Props.filterProps])
  ...
  useEffect(() => {
    if (docdatalst620a8Props?.clearData === true) {
      setdocdatalst620a8([]);
      setAllDataObject([]);
      setAllData([]);
      setSelectedPaginationData([]);
      setLockedData((pre:any) => ({...pre, data:[]}));
      setdocdatalst620a8Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [docdatalst620a8Props?.clearData])
  useEffect(() => {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [docdatalst620a8Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = docdatalst620a8Props?.selectedIds[docdatalst620a8Props?.selectedIds?.length-1]
    if(Array.isArray(docdatalst620a8) && docdatalst620a8.length>0)
    {
      let data = docdatalst620a8[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "",
        value: data[""],
  ...
  useEffect(() => {
    if (docdatalst620a8Props?.selectedIds?.length != 0) handlePrimaryTable()
  }, [docdatalst620a8Props?.selectedIds])
    ...
    <div className='w-full h-full'>
          <div className=' w-full h-full flex flex-row'>
            <Table
              ...
              edgePadding={true}
              selectedIds={docdatalst620a8Props?.selectedIds}  
...
export default Tabledocdatalst
