...
import Tabledocdatalst  from './Tabledocdatalst';  
...
const Groupdocdatalst = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1",componentId:"ba7a656c3d504ab59221ea7c740620a8",from:"GroupDocdatalst",isTable : true,accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("docname")){
      setdocnamefb324({...docnamefb324,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("delete")){
      setdeleted70e2({...deleted70e2,isDisabled:true});
    }
  //////////////
  }
  ...
  const docdatalst620a8Ref = useRef<any>(null);
  const handleClearSearch = () => {
    docdatalst620a8Ref.current?.setSearchParams();
    docdatalst620a8Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(docdatalst620a8) && Object.keys(docdatalst620a8)?.length>0)
      {
        setdocdatalst620a8({})
      }
    }else 
      prevRefreshRef.current= true
  }, [docdatalst620a8Props?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tabledocdatalst headerButtonsRenders={renderBUttons}
        lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={docdatalst620a8Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData}/>}
      </div>
      </div>
    </div>
 )
...
export default Groupdocdatalst
