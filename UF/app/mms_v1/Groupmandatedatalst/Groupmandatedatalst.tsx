...
import Tablemandatedatalst  from './Tablemandatedatalst';  
...
const Groupmandatedatalst = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1",componentId:"e3e6761d4c334522a6aaadac1fe46c27",from:"GroupMandatedatalst",isTable : true,accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("action")){
      setaction53022({...action53022,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("mandateid")){
      setmandateid05ce4({...mandateid05ce4,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("dracntname")){
      setdracntname92ac0({...dracntname92ac0,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("dracntno")){
      setdracntno42c4a({...dracntno42c4a,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("cracntname")){
      setcracntname9fdc4({...cracntname9fdc4,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("cracntno")){
      setcracntnoc7b4d({...cracntnoc7b4d,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("ceilingamnt")){
      setceilingamnt62ee2({...ceilingamnt62ee2,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("frequency")){
      setfrequency1a4a6({...frequency1a4a6,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("duedate")){
      setduedate03f9a({...duedate03f9a,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("expirydate")){
      setexpirydate3fe80({...expirydate3fe80,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("status")){
      setstatus68b01({...status68b01,isDisabled:true});
    }
  //////////////
  }
  ...
  const mandatedatalst46c27Ref = useRef<any>(null);
  const handleClearSearch = () => {
    mandatedatalst46c27Ref.current?.setSearchParams();
    mandatedatalst46c27Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(mandatedatalst46c27) && Object.keys(mandatedatalst46c27)?.length>0)
      {
        setmandatedatalst46c27({})
      }
    }else 
      prevRefreshRef.current= true
  }, [mandatedatalst46c27Props?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-md ${isDark ? 'text-white' : 'text-black'}`}
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
        <div className='flex flex-1 w-full min-h-0'>
       {<Tablemandatedatalst headerButtonsRenders={renderBUttons}
        lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={mandatedatalst46c27Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData}/>}
      </div>
      </div>
    </div>
 )
...
export default Groupmandatedatalst
