...
import Groupmandatedtls  from "../Groupmandatedtls/Groupmandatedtls";
import DocumentViewerdocviewer  from "./DocumentViewerdocviewer";
...
const Groupoverallgroup = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1",componentId:"7512ba7a87c2441a9c7e365031a00e53",from:"GroupOverallgroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("mandatedtls")){
      setmandatedtls2cb9f({...mandatedtls2cb9f,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("docviewer")){
      setdocviewer4f7f9({...docviewer4f7f9,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  function handleConfirmOnLoad(){
    ...
    const handleOnload=()=>{
      ...
      setdynamic_group_btns3c327((prev:any) => ({ ...prev, ...overallgroup00e53 }));
  }
  const handleOnChange=()=>{

  }
  const overallgroup00e53Ref = useRef<any>(null);
  const handleClearSearch = () => {
    overallgroup00e53Ref.current?.setSearchParams();
    overallgroup00e53Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(overallgroup00e53) && Object.keys(overallgroup00e53)?.length>0)
      {
        setoverallgroup00e53({})
      }
    }else 
      prevRefreshRef.current= true
  }, [overallgroup00e53Props?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-2xl p-4 ${isDark ? 'text-white' : 'text-black'}`}
    >
        {allowedComponent.includes("mandatedtls")  &&<Groupmandatedtls  
        ...
        {allowedControls.includes("docviewer") ?<DocumentViewerdocviewer   /* 4f7f9 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}
