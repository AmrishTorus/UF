...
import Tablevaldtndatalst  from './Tablevaldtndatalst';  
...
const Groupvaldtndatalst = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1",componentId:"4e10bcd2c38a46e595b250bb29bd58f5",from:"GroupValdtndatalst",isTable : true,accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("vldcode")){
      setvldcode7ab9e({...vldcode7ab9e,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("vldreason")){
      setvldreason6b5dc({...vldreason6b5dc,isDisabled:true});
    }
  //////////////
  }
  ...
  const valdtndatalstd58f5Ref = useRef<any>(null);
  const handleClearSearch = () => {
    valdtndatalstd58f5Ref.current?.setSearchParams();
    valdtndatalstd58f5Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(valdtndatalstd58f5) && Object.keys(valdtndatalstd58f5)?.length>0)
      {
        setvaldtndatalstd58f5({})
      }
    }else 
      prevRefreshRef.current= true
  }, [valdtndatalstd58f5Props?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tablevaldtndatalst headerButtonsRenders={renderBUttons}
        lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={valdtndatalstd58f5Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData}/>}
      </div>
      </div>
    </div>
 )
...
export default Groupvaldtndatalst
