...
let defaultColumns = [
  {
    "id": "vldcode",
    "nodeid": "7157fd91b562466eb308fb825d67ab9e",
    "name": "Vld Code",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "vldreason",
    "nodeid": "5c8f6a52075c4da2b749dcfd2666b5dc",
    "name": "Vld Reason",
    "meta": {
      "sort": true
    },
...
const Tablevaldtndatalst = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,primaryTableData, setPrimaryTableData,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData }: any)=>{
 ...
 /////////////
  ...
  const {vldcode7ab9e, setvldcode7ab9e}= useContext(TotalContext) as TotalContextProps  
  const {vldreason6b5dc, setvldreason6b5dc}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        componentId: '4e10bcd2c38a46e595b250bb29bd58f5',
        isTable: true,
        from :"Table",
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setvaldtndatalstd58f5Props((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setvaldtndatalstd58f5Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '4e10bcd2c38a46e595b250bb29bd58f5',
                elementId: '4e10bcd2c38a46e595b250bb29bd58f5'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( valdtndatalstd58f5Props?.presetValues&&Object.keys(valdtndatalstd58f5Props?.presetValues).length > 0) {
          filtertedData = [valdtndatalstd58f5Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setvaldtndatalstd58f5(uf_paginationDataFilter.data||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(valdtndatalstd58f5Props.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [valdtndatalstd58f5Props.filterProps])
  ...
  useEffect(() => {
    if (valdtndatalstd58f5Props?.clearData === true) {
      setvaldtndatalstd58f5([]);
      setAllDataObject([]);
      setAllData([]);
      setSelectedPaginationData([]);
      setLockedData((pre:any) => ({...pre, data:[]}));
      setvaldtndatalstd58f5Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [valdtndatalstd58f5Props?.clearData])
  useEffect(() => {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [valdtndatalstd58f5Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = valdtndatalstd58f5Props?.selectedIds[valdtndatalstd58f5Props?.selectedIds?.length-1]
    if(Array.isArray(valdtndatalstd58f5) && valdtndatalstd58f5.length>0)
    {
      let data = valdtndatalstd58f5[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "",
        value: data[""],
  ...
  useEffect(() => {
    if (valdtndatalstd58f5Props?.selectedIds?.length != 0) handlePrimaryTable()
  }, [valdtndatalstd58f5Props?.selectedIds])
  ...
  if (valdtndatalstd58f5?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div className=' w-full h-full flex flex-row'>
            <Table
              ...
              edgePadding={true}
              selectedIds={valdtndatalstd58f5Props?.selectedIds}  
...
export default Tablevaldtndatalst
