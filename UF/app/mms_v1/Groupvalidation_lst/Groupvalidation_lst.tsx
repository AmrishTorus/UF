...
import Groupvaldtn_data_lst  from "../Groupvaldtn_data_lst/Groupvaldtn_data_lst";
...
const Groupvalidation_lst = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:MMS:AFK:Mms_Mandate_Info:AFVK:v1",componentId:"232192b1dffc476c80523e77aec771f5",from:"GroupValidationLst",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("valdtn_data_lst")){
      setvaldtn_data_lst378bc({...valdtn_data_lst378bc,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const validation_lst771f5Ref = useRef<any>(null);
  const handleClearSearch = () => {
    validation_lst771f5Ref.current?.setSearchParams();
    validation_lst771f5Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(validation_lst771f5) && Object.keys(validation_lst771f5)?.length>0)
      {
        setvalidation_lst771f5({})
      }
    }else 
      prevRefreshRef.current= true
  }, [validation_lst771f5Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("valdtn_data_lst")  &&<Groupvaldtn_data_lst  
...
export default Groupvalidation_lst
