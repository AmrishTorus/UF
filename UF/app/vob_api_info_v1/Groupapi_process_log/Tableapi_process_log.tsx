...
let defaultColumns = [
  {
    "id": "trs_created_date",
    "nodeid": "67696f0df9dd4cf09d8f8fe816893619",
    "name": "TimeStamp",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "requestdata",
    "nodeid": "ecbf4b66673e41d295c7e73cf471d4f4",
    "name": "Request",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "responsedata",
    "nodeid": "f0f3514a59f54c518ba624c13aa35a3f",
    "name": "Response",
    "meta": {
      "sort": true
    },
  ...
  },
  {
    ...
    "nodeid": "87fedf00d9ab4aaaa9de452b8ca80eee",
...
const Tableapi_process_log = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,primaryTableData, setPrimaryTableData,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData }: any)=>{
 ...
 /////////////
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setapi_process_log655a7Props((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setapi_process_log655a7Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["tob_api_repositoryid"]:api_process_log655a7Props?.presetValues?._id}    
 
    let dstKey=dfKey?.dfKey
    dstKey=dstKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    try {
    if(!api_process_log655a7Props?.presetValues || Object.keys(api_process_log655a7Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_API_INFO:AFVK:v1:UO', 
                nodeId: 'f03de6a188a541fa9d2984ed872655a7',
                elementId: 'f03de6a188a541fa9d2984ed872655a7'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setapi_process_log655a7(uf_paginationDataFilter.data||[])
...
////////////////////////////////
  const RowAction = ({item,index,nodeName}: any) => {
    ...
    if(handleSecurity("view_logs") &&nodeName?.toLowerCase()=="buttonview_logs"){
      return (
        <React.Fragment>
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(api_process_log655a7))
    {
      GetTableDetails()
    }
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(api_process_log655a7Props.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [api_process_log655a7Props.filterProps])
  ...
  useEffect(() => {
    if (api_process_log655a7Props?.clearData === true) {
      setapi_process_log655a7([]);
      setAllDataObject([]);
      setAllData([]);
      setSelectedPaginationData([]);
      setLockedData((pre:any) => ({...pre, data:[]}));
      setapi_process_log655a7Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [api_process_log655a7Props?.clearData])
  useEffect(() => {
    setapi_process_log655a7([])
    setapi_process_log655a7Props((pre:any)=>({...pre, selectedIds:[]}))
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [api_process_log655a7Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = api_process_log655a7Props?.selectedIds[api_process_log655a7Props?.selectedIds?.length-1]
    if(Array.isArray(api_process_log655a7) && api_process_log655a7.length>0)
    {
      let data = api_process_log655a7[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "id",
        value: data["id"],
  ...
  useEffect(() => {
    if (api_process_log655a7Props?.selectedIds?.length != 0) handlePrimaryTable()
  }, [api_process_log655a7Props?.selectedIds])
  ...
  if (api_process_log655a7?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div className=' w-full h-full flex flex-row'>
            <Table
              ...
              edgePadding={true}
              selectedIds={api_process_log655a7Props?.selectedIds}  
