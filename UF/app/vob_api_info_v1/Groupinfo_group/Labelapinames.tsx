...
const Labelapinames = ({encryptionFlagCompData}:any) => {
  ...
  try{
    ...
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        controlId: "cf6098acc84c43068325bf605d9f3b7f",
        isTable: false,
        from:"label",
...
useEffect(()=>{
  handleMapperValue();
  setinfo_group5e349((pre:any)=>({...pre,apinames:""}));
},[apinamesf3b7f?.refresh])


const handleClick =async(e:any)=>{
  setinfo_group5e349((prev: any) => ({ ...prev, apinames: e.target.value }));
  let code = allCode;
    if (code != '') {
    let codeStates: any = {};
  ...
  if (apinamesf3b7f?.isHidden) {
    return <></>
  }  

  return (
    <div 
      style={{gridColumn: `1 / 5`,gridRow: `1 / 11`, gap:``, height: `100%`, overflow: 'hidden'}} >

      <Label 
        className=""
        disabled= {apinamesf3b7f?.isDisabled ? true : false}
        theme="normal"
        interactive={false}
      onClick = {handleClick}
      >
