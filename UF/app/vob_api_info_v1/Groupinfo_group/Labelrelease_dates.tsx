  ...
  try{
    ...
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        controlId: "ca522ed55bae41ae87736306b70d97a1",
        isTable: false,
        from:"label",
...
useEffect(()=>{
  handleMapperValue();
  setinfo_group5e349((pre:any)=>({...pre,release_dates:""}));
},[release_datesd97a1?.refresh])


const handleClick =async(e:any)=>{
  setinfo_group5e349((prev: any) => ({ ...prev, release_dates: e.target.value }));
  let code = allCode;
    if (code != '') {
    let codeStates: any = {};
  ...
  if (release_datesd97a1?.isHidden) {
    return <></>
  }  

  return (
    <div 
      style={{gridColumn: `17 / 21`,gridRow: `1 / 11`, gap:``, height: `100%`, overflow: 'hidden'}} >

      <Label 
        className=""
        disabled= {release_datesd97a1?.isDisabled ? true : false}
        theme="normal"
        interactive={false}
      onClick = {handleClick}
      >
