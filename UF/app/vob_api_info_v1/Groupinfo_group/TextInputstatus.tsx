...
const TextInputstatus = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT009:FNGK:AF:FNK:DF-DFD:CATK:TOB001:AFGK:TOB002:AFK:Mongo_API_Repository_DFD:AFVK:v1|fcdd4a9059f84161990c20c300139cd7|properties.status"
      ],
      "targetKey": "CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_API_INFO:AFVK:v1|f0167144898c456bb433c00a9b75e349|06122b1bab2142fbb5fe1df6e25ddf07"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,VOB_API_INFO_v1:{...pre?.VOB_API_INFO_v1,status:undefined}}));
    ...
    setinfo_group5e349((prev: any) => ({ ...prev, status: +e.target.value }));
    }
    else{
    setinfo_group5e349((prev: any) => ({ ...prev, status: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['api_info_group']  = {...api_info_groupd3ad5,status:newInputValue},
      codeStates['setapi_info_group'] = setapi_info_groupd3ad5,
      codeStates['info_group']  = {...info_group5e349,status:newInputValue},
      codeStates['setinfo_group'] = setinfo_group5e349,
      codeStates['info_summary_group']  = {...info_summary_groupa2a0f,status:newInputValue},
      codeStates['setinfo_summary_group'] = setinfo_summary_groupa2a0f,
      codeStates['api_process_log_group']  = {...api_process_log_groupff19c,status:newInputValue},
      codeStates['setapi_process_log_group'] = setapi_process_log_groupff19c,
      codeStates['api_process_log']  = {...api_process_log655a7,status:newInputValue},
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "06122b1bab2142fbb5fe1df6e25ddf07",
          isTable: false,
  ...
  if(dfd_mongo_api_repository_dfd_v1Props?.setSearchFilters && dfd_mongo_api_repository_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_mongo_api_repository_dfd_v1Props.data) && dfd_mongo_api_repository_dfd_v1Props.data.length > 0){
      setinfo_group5e349((pre:any)=>({...pre,status:dfd_mongo_api_repository_dfd_v1Props.data[0]?.status}));
    }
  }
  },[dfd_mongo_api_repository_dfd_v1Props?.setSearchFilters])
  if (statusddf07?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `9 / 13`,gridRow: `13 / 23`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value= {dynamicStateandType.type === "number" ? formatNumberWithCommas(info_group5e349?.status) || "" : info_group5e349?.status|| ""}
         disabled= {statusddf07?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.VOB_API_INFO_v1?.status ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
