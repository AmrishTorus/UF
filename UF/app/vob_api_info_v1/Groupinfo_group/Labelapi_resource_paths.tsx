...
const Labelapi_resource_paths = ({encryptionFlagCompData}:any) => {
  ...
  try{
    ...
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        controlId: "c5104c49c2154cc48478ad5e7c86c67f",
        isTable: false,
        from:"label",
...
useEffect(()=>{
  handleMapperValue();
  setinfo_group5e349((pre:any)=>({...pre,api_resource_paths:""}));
},[api_resource_paths6c67f?.refresh])


const handleClick =async(e:any)=>{
  setinfo_group5e349((prev: any) => ({ ...prev, api_resource_paths: e.target.value }));
  let code = allCode;
    if (code != '') {
    let codeStates: any = {};
  ...
  if (api_resource_paths6c67f?.isHidden) {
    return <></>
  }  

  return (
    <div 
      style={{gridColumn: `21 / 25`,gridRow: `1 / 11`, gap:``, height: `100%`, overflow: 'hidden'}} >

      <Label 
        className=""
        disabled= {api_resource_paths6c67f?.isDisabled ? true : false}
        theme="normal"
        interactive={false}
      onClick = {handleClick}
      >
...
export default Labelapi_resource_paths
