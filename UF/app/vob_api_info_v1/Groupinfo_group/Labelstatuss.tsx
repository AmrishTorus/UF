  ...
  try{
    ...
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        controlId: "321c19c31480420f98cafa0ab386320b",
        isTable: false,
        from:"label",
...
useEffect(()=>{
  handleMapperValue();
  setinfo_group5e349((pre:any)=>({...pre,statuss:""}));
},[statuss6320b?.refresh])


const handleClick =async(e:any)=>{
  setinfo_group5e349((prev: any) => ({ ...prev, statuss: e.target.value }));
  let code = allCode;
    if (code != '') {
    let codeStates: any = {};
  ...
  if (statuss6320b?.isHidden) {
    return <></>
  }  

  return (
    <div 
      style={{gridColumn: `9 / 13`,gridRow: `1 / 11`, gap:``, height: `100%`, overflow: 'hidden'}} >

      <Label 
        className=""
        disabled= {statuss6320b?.isDisabled ? true : false}
        theme="normal"
        interactive={false}
      onClick = {handleClick}
      >
