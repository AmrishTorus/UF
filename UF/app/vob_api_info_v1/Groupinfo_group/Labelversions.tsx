  ...
  try{
    ...
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        controlId: "54563b646c484d25beeea589e0a69477",
        isTable: false,
        from:"label",
...
useEffect(()=>{
  handleMapperValue();
  setinfo_group5e349((pre:any)=>({...pre,versions:""}));
},[versions69477?.refresh])


const handleClick =async(e:any)=>{
  setinfo_group5e349((prev: any) => ({ ...prev, versions: e.target.value }));
  let code = allCode;
    if (code != '') {
    let codeStates: any = {};
  ...
  if (versions69477?.isHidden) {
    return <></>
  }  

  return (
    <div 
      style={{gridColumn: `5 / 9`,gridRow: `1 / 11`, gap:``, height: `100%`, overflow: 'hidden'}} >

      <Label 
        className=""
        disabled= {versions69477?.isDisabled ? true : false}
        theme="normal"
        interactive={false}
      onClick = {handleClick}
      >
