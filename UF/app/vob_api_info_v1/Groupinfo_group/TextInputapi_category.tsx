...
const TextInputapi_category = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT009:FNGK:AF:FNK:DF-DFD:CATK:TOB001:AFGK:TOB002:AFK:Mongo_API_Repository_DFD:AFVK:v1|fcdd4a9059f84161990c20c300139cd7|properties.api_category"
      ],
      "targetKey": "CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_API_INFO:AFVK:v1|f0167144898c456bb433c00a9b75e349|62d40157111e47e8ad24589267495348"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'api_category',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,VOB_API_INFO_v1:{...pre?.VOB_API_INFO_v1,api_category:undefined}}));
    ...
    setinfo_group5e349((prev: any) => ({ ...prev, api_category: +e.target.value }));
    }
    else{
    setinfo_group5e349((prev: any) => ({ ...prev, api_category: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['api_info_group']  = {...api_info_groupd3ad5,api_category:newInputValue},
      codeStates['setapi_info_group'] = setapi_info_groupd3ad5,
      codeStates['info_group']  = {...info_group5e349,api_category:newInputValue},
      codeStates['setinfo_group'] = setinfo_group5e349,
      codeStates['info_summary_group']  = {...info_summary_groupa2a0f,api_category:newInputValue},
      codeStates['setinfo_summary_group'] = setinfo_summary_groupa2a0f,
      codeStates['api_process_log_group']  = {...api_process_log_groupff19c,api_category:newInputValue},
      codeStates['setapi_process_log_group'] = setapi_process_log_groupff19c,
      codeStates['api_process_log']  = {...api_process_log655a7,api_category:newInputValue},
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "62d40157111e47e8ad24589267495348",
          isTable: false,
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'api_category', type: 'number'});
      }
      ...
      //   type={
      //     name:'api_category',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.api_category.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.api_category.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.api_category.type
      //   }
      ...
      //   type={
      //     name:'api_category',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.api_category.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.api_category.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.api_category.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  if(dfd_mongo_api_repository_dfd_v1Props?.setSearchFilters && dfd_mongo_api_repository_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_mongo_api_repository_dfd_v1Props.data) && dfd_mongo_api_repository_dfd_v1Props.data.length > 0){
      setinfo_group5e349((pre:any)=>({...pre,api_category:dfd_mongo_api_repository_dfd_v1Props.data[0]?.api_category}));
    }
  }
  },[dfd_mongo_api_repository_dfd_v1Props?.setSearchFilters])
  if (api_category95348?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 17`,gridRow: `13 / 23`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value= {dynamicStateandType.type === "number" ? formatNumberWithCommas(info_group5e349?.api_category) || "" : info_group5e349?.api_category|| ""}
         disabled= {api_category95348?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.VOB_API_INFO_v1?.api_category ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
