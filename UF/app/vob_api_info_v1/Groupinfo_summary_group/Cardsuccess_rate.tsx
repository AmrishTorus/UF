...
const Cardsuccess_rate = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async():Promise<void>=>{
    try{
    let code:string;
    const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_API_INFO:AFVK:v1",  componentId:"a7e5b7356fa848f2b4e46119e6fa2a0f",controlId:"4012f17c0dbc48a08464ec0763dee58b",isTable:false,accessProfile:accessProfile,from:"cardSucess Rate"},{
      headers: {
    ...
    try{
      ...
      setinfo_summary_groupa2a0f((pre:any)=>({...pre,success_rate:dfd_mongo_totalcalls_dfd_v1Props[0]?.success_rate}));
  }
    }catch(err){
      console.log(err);
    }
...
useEffect(() => {
  ...
  if (success_rateee58b?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `9 / 17`,gridRow: `1 / 27`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
      style={style}
      className="p-1 !text-lg"   
      theme="success"
      view="filled"
      label={keyset("Sucess Rate")}
      disabled= {success_rateee58b?.isDisabled ? true : false}
      onClick={handleClick} 
      contentAlign={"center"}
      >
      {info_summary_groupa2a0f?.success_rate?info_summary_groupa2a0f?.success_rate:"0"}
      </Card>
    </div>
  )
}
