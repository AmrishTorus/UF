...
const Cardtotal_calls = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async():Promise<void>=>{
    try{
    let code:string;
    const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT009:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:VOB_API_INFO:AFVK:v1",  componentId:"a7e5b7356fa848f2b4e46119e6fa2a0f",controlId:"daa273d0c51f4fdeb7f1a668a1c5c0ea",isTable:false,accessProfile:accessProfile,from:"cardTotal Calls"},{
      headers: {
    ...
    try{
      ...
      setinfo_summary_groupa2a0f((pre:any)=>({...pre,total_calls:dfd_mongo_totalcalls_dfd_v1Props[0]?.total_calls}));
  }
    }catch(err){
      console.log(err);
    }
...
useEffect(() => {
  ...
  if (total_calls5c0ea?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `1 / 9`,gridRow: `1 / 27`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
      style={style}
      ...
      disabled= {total_calls5c0ea?.isDisabled ? true : false}
      onClick={handleClick} 
      contentAlign={"center"}
      >
      {info_summary_groupa2a0f?.total_calls?info_summary_groupa2a0f?.total_calls:"0"}
      </Card>
    </div>
  )
}
